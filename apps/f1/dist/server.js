...
import { getDefaultReposDir, getDefaultWorktreesDir, } from "cyrus-core";
 ...
 */
function createEdgeWorkerConfig() {
    ...
    const config = {
        ...
        ...(process.env.CYRUS_DEFAULT_RUNNER && {
            defaultRunner: process.env.CYRUS_DEFAULT_RUNNER,
        }),
...
function displayConnectionInfo() {
    ...
    console.log("");
    console.log(`  ${cyan("Server:")}    ${bold(`http://localhost:${CYRUS_PORT}`)}`);
    console.log(`  ${cyan("RPC:")}       ${bold(`http://localhost:${CYRUS_PORT}/cli/rpc`)}`);
    ...
    if (MULTI_REPO_MODE) {
        console.log(`  ${cyan("Multi-Repo:")} ${bold("enabled")} (${dim(CYRUS_REPO_PATH_2 || "")})`);
        console.log(dim("  Routing context will be included in orchestrator prompts"));
    }
    console.log("");
