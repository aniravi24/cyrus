...
export function createCreateCommentCommand() {
    ...
    cmd
        ...
        try {
            const result = await rpcCall("createComment", params);
            console.log(success("Comment created successfully"));
            console.log(`  ${formatKeyValue("Comment ID", result.comment.id)}`);
            console.log(`  ${formatKeyValue("Body", result.comment.body.slice(0, 60) + (result.comment.body.length > 60 ? "..." : ""))}`);
            console.log(`  ${formatKeyValue("Created", new Date(result.comment.createdAt).toISOString())}`);
