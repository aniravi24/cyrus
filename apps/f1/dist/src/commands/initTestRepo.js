...
import { GITIGNORE_TEMPLATE, INDEX_TS_TEMPLATE, PACKAGE_JSON_TEMPLATE, RATE_LIMITER_TS_TEMPLATE, README_MD_TEMPLATE, TSCONFIG_JSON_TEMPLATE, TYPES_TS_TEMPLATE, } from "../templates/index.js";
import { dim, error, success } from "../utils/colors.js";
export function createInitTestRepoCommand() {
    const cmd = new Command("init-test-repo");
    cmd
        .description("Initialize a test repository with a partially-complete rate limiter library")
        .requiredOption("-p, --path <directory>", "Target directory path for the test repository")
        ...
        if (existsSync(targetPath)) {
            ...
            console.error("  Please choose a different path or remove the existing directory.");
            process.exit(1);
        }
        try {
            ...
            execSync('git commit -m "Initial commit: rate limiter library scaffold"', { cwd: targetPath, stdio: "pipe" });
            ...
            console.log("The repository contains a partially-complete rate limiter library:");
            ...
            if (err instanceof Error) {
                console.error(error(`Failed to create test repository: ${err.message}`));
            }
            else {
                console.error(error("Failed to create test repository: Unknown error"));
            }
            process.exit(1);
        }
    });
...
//# sourceMappingURL=initTestRepo.js.map