...
import { formatKeyValue, formatTimestamp, printTable, } from "../utils/output.js";
import { printRpcUrl, rpcCall } from "../utils/rpc.js";
export function createViewSessionCommand() {
    ...
    cmd
        ...
        .option("-l, --limit <number>", "Maximum number of activities to return", "50")
        ...
        try {
            const result = await rpcCall("viewSession", params);
            console.log(success("Session Details"));
            console.log(`  ${formatKeyValue("Session ID", result.session.sessionId)}`);
            console.log(`  ${formatKeyValue("Issue ID", result.session.issueId)}`);
            console.log(`  ${formatKeyValue("Status", result.session.status)}`);
            console.log(`  ${formatKeyValue("Created", formatTimestamp(new Date(result.session.createdAt).toISOString()))}`);
            console.log(`  ${formatKeyValue("Updated", formatTimestamp(new Date(result.session.updatedAt).toISOString()))}`);
            console.log(`  ${formatKeyValue("Total Activities", result.totalCount)}`);
...
//# sourceMappingURL=viewSession.js.map