...
export function createAssignIssueCommand() {
    ...
    cmd
        ...
        .requiredOption("-a, --assignee-id <id>", "User ID to assign to (use 'userId' not 'assigneeId' in params)")
        ...
        try {
            const result = await rpcCall("assignIssue", params);
            console.log(success("Issue assigned successfully"));
            console.log(`  ${formatKeyValue("Issue ID", result.issue.id)}`);
            console.log(`  ${formatKeyValue("Identifier", result.issue.identifier)}`);
            console.log(`  ${formatKeyValue("Title", result.issue.title)}`);
            console.log(`  ${formatKeyValue("Assignee ID", result.issue.assigneeId ?? "none")}`);
        }
        catch (err) {
