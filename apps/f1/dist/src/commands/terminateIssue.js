...
export function createTerminateIssueCommand() {
    const cmd = new Command("terminate-issue");
    cmd
        .description("Terminate an issue (completed / canceled / deleted). Triggers EdgeWorker terminal-state cleanup including cyrus-teardown.sh.")
        .requiredOption("-i, --issue-id <id>", "Issue ID to terminate")
        .option("-a, --action <action>", "Terminal action: completed | canceled | deleted", "completed")
        ...
        if (action !== "completed" &&
            action !== "canceled" &&
            action !== "deleted") {
            console.error(error(`Invalid --action: ${options.action}. Must be one of: completed, canceled, deleted`));
            process.exit(1);
        }
        const params = {
        ...
        try {
            const result = await rpcCall("terminateIssue", params);
            ...
            if (err instanceof Error) {
                ...
                console.error("    - The action is one of: completed, canceled, deleted");
...
//# sourceMappingURL=terminateIssue.js.map