...
function splitThreadKey(threadKey) {
    const idx = threadKey.lastIndexOf(":");
    if (idx <= 0 || idx === threadKey.length - 1) {
        throw new Error(`Invalid thread key '${threadKey}'. Expected '<channel>:<ts>' (e.g. 'C_TEST:1701234567.000100').`);
    }
    return {
...
export function createPromptChatThreadCommand() {
    const cmd = new Command("prompt-chat-thread");
    cmd
        .description("Send an additional message to an existing chat thread (mid-flight prompt)")
        .requiredOption("-k, --thread-key <key>", "Thread key (e.g. 'C_TEST:1234567890.000') from start-chat-session or list-chat-threads")
        .requiredOption("-t, --text <text>", "Message text to send")
        .option("-u, --user <id>", "Slack user ID (default: U_F1_USER)", "U_F1_USER")
        ...
        try {
            ...
            if (err instanceof Error) {
                console.error(error(`Failed to prompt chat thread: ${err.message}`));
                process.exit(1);
            }
            throw err;
        }
...
//# sourceMappingURL=promptChatThread.js.map