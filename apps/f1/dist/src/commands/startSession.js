...
export function createStartSessionCommand() {
    ...
    cmd
        ...
        try {
            const result = await rpcCall("startSession", params);
            ...
            console.log(`  ${formatKeyValue("Created At", new Date(result.session.createdAt).toISOString())}`);
        }
        catch (err) {
            if (err instanceof Error) {
                ...
                console.error("    - There isn't already an active session on this issue");
