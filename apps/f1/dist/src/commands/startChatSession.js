...
export function createStartChatSessionCommand() {
    const cmd = new Command("start-chat-session");
    cmd
        .description("Dispatch a synthetic Slack app_mention event into the EdgeWorker chat handler")
        .option("-c, --channel <id>", "Slack channel ID (default: C_F1_CHAN)", "C_F1_CHAN")
        .option("-u, --user <id>", "Slack user ID issuing the mention (default: U_F1_USER)", "U_F1_USER")
        .option("-t, --text <text>", "Message text (default: hello)", "hello")
        .option("-T, --thread-ts <ts>", "Thread timestamp to reuse an existing chat thread")
        ...
        try {
            ...
            if (err instanceof Error) {
                console.error(error(`Failed to dispatch chat event: ${err.message}`));
                process.exit(1);
            }
            throw err;
        }
...
//# sourceMappingURL=startChatSession.js.map