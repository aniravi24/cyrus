...
program
    ...
    .option("--cyrus-home <path>", "Specify custom Cyrus config directory", resolve(homedir(), ".cyrus"))
...
program
    ...
    .description('Clone a repo and add it to config. URL accepts any valid git clone address (e.g., "https://github.com/org/repo.git"). Workspace is the display name of the Linear workspace (e.g., "My Workspace"). If URL is omitted, prompts interactively.')
    .option("-l, --label <labels>", "Comma-separated routing labels (defaults to repo name)")
    .option("-b, --base-branch <branch>", "Base branch name (auto-detected from remote if not specified)")
    .action(async (url, workspace, cmdOpts) => {
    const opts = program.opts();
    const app = new Application(opts.cyrusHome, opts.envFile, packageJson.version);
    const args = [url, workspace].filter(Boolean);
    if (cmdOpts.label) {
