...
import { DEFAULT_CONFIG_FILENAME, migrateEdgeConfig, } from "cyrus-core";
...
export class SelfAuthCommand extends BaseCommand {
    server = null;
    callbackPort = parseInt(process.env.CYRUS_SERVER_PORT || "3456", 10);
    async execute(_args) {
        ...
        let config;
        try {
            config = migrateEdgeConfig(JSON.parse(readFileSync(configPath, "utf-8")));
        }
        catch {
        ...
        try {
            if (process.env.CLOUDFLARE_TOKEN) {
                ...
                const sharedApplicationServer = new SharedApplicationServer(this.callbackPort, baseUrl, false);
                await sharedApplicationServer.startCloudflareTunnel(process.env.CLOUDFLARE_TOKEN);
            }
            ...
            if (!config.repositories || config.repositories.length === 0) {
                console.log("   No repositories configured yet. Run 'cyrus self-add-repo' to add one.");
            }
            console.log();
            this.logSuccess("Authentication complete! Restart cyrus to use the new tokens.");
            process.exit(0);
        }
        catch (error) {
