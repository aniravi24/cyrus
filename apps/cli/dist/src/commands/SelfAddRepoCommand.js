...
import { DEFAULT_BASE_BRANCH, DEFAULT_CONFIG_FILENAME, migrateEdgeConfig, } from "cyrus-core";
 ...
 */
export function detectDefaultBranch(repositoryPath) {
    try {
    ...
    async execute(args) {
        ...
        try {
            ...
            if (workspaces.size === 0) {
                this.logError("No Linear credentials found. Run 'cyrus self-auth-linear' first.");
                process.exit(1);
            }
            // Get workspace
            ...
            // Clone the repo
            const repositoryPath = resolve(getDefaultReposDir(this.app.cyrusHome), repoName);
