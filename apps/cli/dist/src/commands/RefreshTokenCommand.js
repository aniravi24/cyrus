    ...
    async execute(_args) {
        ...
        if (config.linearWorkspaces) {
            for (const [wsId, wsConfig] of Object.entries(config.linearWorkspaces)) {
                ...
                console.log(`${workspaceStatuses.length}. Workspace ${name}: ${result.valid ? "✅ Valid" : "❌ Invalid"}`);
            }
        }
        if (workspaceStatuses.length === 0) {
            this.logError("No Linear workspaces configured. Run 'cyrus self-auth-linear' first.");
            process.exit(1);
        }
        ...
        if (answer.toLowerCase() === "all") {
            indicesToRefresh.push(...Array.from({ length: workspaceStatuses.length }, (_, i) => i));
        }
        else {
