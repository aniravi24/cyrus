 ...
 */
function inferSessionSource(sessionId) {
    if (sessionId.startsWith("github-"))
        return "github";
    if (sessionId.startsWith("gitlab-"))
        return "gitlab";
    if (sessionId.startsWith("slack-"))
        return "slack";
    return "linear";
}
function normalize(resolved) {
...
export function registerLogFailureModeTool(server, options) {
    server.registerTool("log_failure_mode", {
        description: "Log a material, user-visible agent failure to the Cyrus internal failure-modes Linear project. Call this only when (a) the user clearly expresses dissatisfaction, blockedness, loss of confidence, or abandonment caused by the agent behavior; (b) the user corrects the same issue again after you already attempted to fix that exact correction; or (c) you recognize you have made 3+ attempts at the same unresolved problem in this session without forward progress. Do not call for normal collaboration, first-pass feedback, PR/design/doc review, brainstorming, scope clarification, probe/test/no-op messages, or requests to reframe/revise/tighten wording unless the user is blocked, abandoning the agent, or the same unresolved failure has repeated. The recap should describe what the user asked for vs. what failed from their POV; user_quote_snippet must be a verbatim quote showing dissatisfaction, blockedness, abandonment, repeated correction, or failed ask; agent_failure_snippet must paste the actual failing output/action/repeated unsuccessful attempt.",
        inputSchema: {
            cwd: z
                .string()
                .describe("The current working directory of this agent session. Used to resolve the session id internally."),
            category: z
                .string()
                .min(1)
                .describe("Short free-form category name (e.g. 'screenshots-not-returned', 'port-conflict', 'wrong-file-edited'). Pick something concise and reusable — patterns will emerge over time."),
            recap: z
                .string()
                .min(1)
                .describe("What the user asked for vs. what failed, in their POV. 1-3 sentences."),
            user_quote_snippet: z
                .string()
                .optional()
                .describe("Verbatim quote of the user's ask or dissatisfaction. Strongly preferred but optional — if you can't capture a clean quote (e.g. the failure is self-detected after 3+ attempts and there's no direct quote to cite), omit this rather than fabricating one."),
            agent_failure_snippet: z
                .string()
                .optional()
                .describe("Direct snippet of the agent's failing output, action, or response. Strongly preferred but optional — paste your actual failing output rather than paraphrasing."),
        },
    }, async ({ cwd, category, recap, user_quote_snippet, agent_failure_snippet, }) => {
...
//# sourceMappingURL=log-failure-mode.js.map