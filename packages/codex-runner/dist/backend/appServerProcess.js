...
class PooledAppServerProcess {
    ...
    constructor(launchOptions, clientFactory, idleCloseMs, 
    ...
    async acquireLease() {
        if (this.disposed) {
            throw new Error("Cannot acquire a lease on a disposed app-server process");
        }
        this.leaseCount += 1;
        this.clearIdleTimer();
        let released = false;
        ...
        return {
            ...
            registerThread: (threadId, handler) => {
                ...
                if (existing && existing !== handler) {
                    throw new Error(`Cannot register Codex thread ${threadId}: already registered`);
                }
...
export class AppServerProcessManager {
    ...
    requestTimeoutMs;
    idleCloseMs;
    constructor(clientFactory = (options) => new AppServerClient(options), options) {
        this.clientFactory = clientFactory;
    ...
    async acquire(config) {
        ...
        if (!proc || proc.isDisposed()) {
            const created = new PooledAppServerProcess(launchOptions, this.clientFactory, this.idleCloseMs, () => {
...
function buildLaunchKey(options) {
