 ...
 * `codexPathOverride`.
 */
export function resolveCodexAppServerLaunch(override) {
    if (override) {
    ...
    try {
        ...
        throw new Error(`Unable to locate ${CODEX_NPM_NAME}. Ensure it is installed as a dependency.`);
    }
    const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8"));
    const binRelative = typeof pkg.bin === "string" ? pkg.bin : pkg.bin?.codex;
    if (!binRelative) {
        throw new Error(`${CODEX_NPM_NAME} has no \`codex\` bin entry; cannot locate the CLI launcher.`);
    }
...
//# sourceMappingURL=codexBinary.js.map