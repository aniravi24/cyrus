 ...
 */
export function ompAgentsDir(env) {
 ...
 */
export function stageOmpAgents(pluginPaths, env) {
    ...
    for (const pluginPath of pluginPaths) {
        ...
        for (const entry of entries) {
            const parsed = parseFrontmatter(readFileSync(join(sourceDir, entry), "utf8"));
            ...
            if (effort)
                lines.push(`thinking-level: ${effort}`);
            for (const [key, value] of Object.entries(parsed.fields)) {
                if (["description", "effort", "model", "name", "thinking-level"].includes(key)) {
                    continue;
                }
...
//# sourceMappingURL=OmpAgentStager.js.map