 ...
 */
export function resolveMcpPolicy(allowedTools, cwd) {
    const discovered = serversDefinedIn(cwd);
    if (!allowedTools)
        return { deniedTools: [], disabledServers: [] };
    const wholeServers = new Set();
    const partialServers = new Set();
    for (const entry of allowedTools) {
        ...
        if (!server)
            continue;
        if (tool)
            partialServers.add(server);
        else
            wholeServers.add(server);
    }
...
//# sourceMappingURL=OmpMcpPolicy.js.map