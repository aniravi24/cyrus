    ...
    if (!VALID_SKILL_NAME.test(name)) {
        return {
            ...
            error: {
                ...
                details: "Skill names may only contain lowercase letters, numbers, hyphens, and underscores.",
            },
        };
    }
 ...
 */
function resolveSkillDir(cyrusHome, skillName) {
    ...
    if (!skillDir.startsWith(`${skillsRoot}/`)) {
        return {
            error: {
                success: false,
                error: "Invalid skill name",
                details: "Skill name must not contain path separators or traversal sequences.",
            },
        };
    }
 ...
 */
function normalizeSkillScope(payload) {
    const clean = (values) => {
 ...
 */
export async function handleUpdateSkill(payload, cyrusHome) {
    try {
 ...
 */
export async function handleDeleteSkill(payload, cyrusHome) {
    try {
        const nameResult = validateSkillName(payload.name);
        if (!nameResult.valid)
            return nameResult.error;
 ...
 */
export async function handleListSkills(_payload, cyrusHome) {
    try {
