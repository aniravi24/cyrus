...
export async function handleRepository(payload, cyrusHome) {
    try {
        ...
        const usePushedTokens = Boolean(tokenStore.getTokenForRepoUrl(payload.repository_url) ??
 ...
 */
export async function handleRepositoryDelete(payload, cyrusHome) {
    try {
        // Validate payload
        if (!payload.repository_name ||
            ...
            return {
                ...
                details: "Please provide a valid repository name to delete (e.g., 'my-repo')",
            };
        }
        ...
        if (payload.linear_team_key) {
            ...
            if (existsSync(teamRepoWorkspaceDir)) {
                try {
                    ...
                    console.warn(`Failed to delete worktrees at ${teamRepoWorkspaceDir}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
        return {
