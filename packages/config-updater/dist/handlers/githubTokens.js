...
import { GitHubTokensPayloadSchema, } from "../types.js";
/** Path of a bundled script within this package's scripts/ directory */
function bundledScriptPath(scriptName) {
 ...
 */
export function ensureGhTokenResolver(cyrusHome) {
 ...
 */
export function ensureGitHubCredentialHelper(cyrusHome) {
    const scriptDir = join(cyrusHome, "scripts");
 ...
 */
export function ensureGhWrapperSupportsCyrusToken(homeDir = homedir()) {
 ...
 */
export function configureGhCliAuth(token) {
 ...
 */
export async function handleGitHubTokens(rawPayload, cyrusHome) {
    ...
    try {
        ...
        console.warn("[githubTokens] gh token resolver install failed:", error instanceof Error ? error.message : String(error));
    }
    try {
        ...
        console.warn("[githubTokens] gh wrapper self-heal failed:", error instanceof Error ? error.message : String(error));
    }
    ...
    if (firstToken) {
        try {
            ...
            console.warn("[githubTokens] gh CLI auth refresh failed:", error instanceof Error ? error.message : String(error));
        }
    }
    return {
        success: true,
