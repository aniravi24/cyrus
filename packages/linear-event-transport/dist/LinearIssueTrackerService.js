...
export class LinearIssueTrackerService {
     ...
     */
    constructor(linearClient, oauthConfig) {
                ...
                try {
                    ...
                    if (!refreshPromise) {
                        refreshPromise = this.doTokenRefresh().catch((refreshError) => {
                            ...
                            refreshPromise = null;
                            console.error("[LinearIssueTrackerService] Token refresh failed:", refreshError);
                            throw refreshError;
                        });
                    }
                    try {
    ...
    async doTokenRefresh() {
        ...
        if (pendingRefresh) {
            console.log(`[LinearIssueTrackerService] Coalescing token refresh for workspace ${workspaceId}`);
    ...
    async executeTokenRefresh() {
        ...
        console.log(`[LinearIssueTrackerService] Refreshing token for workspace ${workspaceId}...`);
        ...
        if (onTokenRefresh) {
            try {
                ...
                console.error("[LinearIssueTrackerService] onTokenRefresh callback failed:", err);
            }
        }
        console.log(`[LinearIssueTrackerService] ✅ Token refreshed successfully for workspace ${workspaceId}`);
