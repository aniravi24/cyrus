...
export class SkillsPluginResolver {
    ...
    async ensureUserPluginScaffolded() {
        ...
        this.logger.info(`Scaffolded user-skills plugin manifest at ${manifestPath}`);
    }
    /**
     ...
     */
    async discoverSkillNames(plugins, context) {
        const skillNames = [];
            ...
            for (const entry of entries) {
                if (context) {
                    ...
                    if (!this.scopeMatches(scope, context)) {
                        this.logger.debug(`Skill "${entry}" excluded by scope filter for current session`);
                        continue;
                    }
                }
     ...
     */
    async readSkillDirEntries(skillsDir) {
        try {
     ...
     */
    async loadSkillScope(skillsDir, skillName) {
        ...
        try {
            ...
            if (repos)
                scope.repositoryIds = repos;
            if (teams)
                scope.linearTeamIds = teams;
            if (labels)
                scope.linearLabelIds = labels;
            ...
            this.logger.warn(`Failed to parse scope.json for skill "${skillName}" — treating as global`, { error: error instanceof Error ? error.message : String(error) });
            return null;
        }
    }
    /**
     ...
     */
    async buildSkillsGuidance(plugins, context) {
        const resolvedPlugins = plugins ?? (await this.resolve());
        const availableSkills = await this.discoverSkillNames(resolvedPlugins, context);
