...
import { buildPromptText, ZulipMessageService, } from "cyrus-zulip-event-transport";
...
export class ZulipChatAdapter {
     ...
     */
    async fetchThreadContext(event, sinceTs) {
        const { message } = event;
        if (message.type !== "stream") {
            return "";
        }
        ...
        if (!channel) {
            this.logger.warn(`Cannot fetch Zulip topic context: no channel name on message ${message.id}`);
            return null;
        }
        ...
        try {
            ...
            const omittedEarlier = messages.length >= TOPIC_CONTEXT_MESSAGE_LIMIT &&
                oldestId !== undefined &&
            ...
            const preamble = sinceId === undefined
                ? ""
            ...
            const truncationNote = omittedEarlier
                ...
                : "";
            return `${preamble}${truncationNote}${this.formatTopicContext(relevant, event.botEmail)}`;
        }
        catch (error) {
            this.logger.warn(`Failed to fetch Zulip topic context: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }
    }
    buildSystemPrompt(event) {
        const repositoryPaths = Array.from(new Set(this.repositoryProvider.getRepositoryPaths().filter(Boolean))).sort();
        const repositoryAccessSection = repositoryPaths.length > 0
            ? `
## Repository Access
...
- No repository paths are configured for this chat session.`;
        const { message } = event;
        const location = message.type === "stream"
...
- Do not use headers (\`#\`). Zulip messages are short; use \`**bold**\` on its own line to separate sections.`;
    ...
    async postReply(event, runner) {
        try {
            ...
            this.logger.info(`Posted Zulip reply (thread ${this.getThreadKey(event)})`);
        }
        catch (error) {
            this.logger.error("Failed to post Zulip reply", error instanceof Error ? error : new Error(String(error)));
        }
    }
    async acknowledgeReceipt(event) {
        await this.messageService.addReaction(event.credentials, event.message.id, RECEIPT_REACTION);
    }
    /**
    ...
    async acknowledgeProcessed(event) {
        ...
        await this.messageService.removeReaction(event.credentials, event.message.id, RECEIPT_REACTION);
        await this.messageService.addReaction(event.credentials, event.message.id, PROCESSED_REACTION);
    }
    async postNotice(event, text) {
        await this.messageService.postMessage({
            credentials: event.credentials,
            destination: this.destinationFor(event.message),
            content: text,
    ...
    async notifyBusy(event) {
        await this.postNotice(event, "I'm still working on the previous request in this topic. I'll pick up your new message once I'm done.");
    }
    /** Where a reply to this message should go */
    destinationFor(message) {
    ...
    formatTopicContext(messages, botEmail) {
        const formattedMessages = messages
            .map((msg) => {
            const author = msg.sender_email === botEmail
                ? "assistant (you)"
...
//# sourceMappingURL=ZulipChatAdapter.js.map