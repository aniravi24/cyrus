        ...
        try {
            ...
            if (maintenance?.enabled) {
                if (!this.threadSessions.has(threadKey) &&
                    ...
                    this.logger.info(`Ignoring non-initiating ${this.adapter.platformName} event for unbound thread ${threadKey} (maintenance mode)`);
                    return;
                }
                this.logger.info(`Maintenance mode is on; declining ${this.adapter.platformName} event for thread ${threadKey}`);
