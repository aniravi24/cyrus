...
import { createLogger, getDefaultWorktreesDir } from "cyrus-core";
...
function isNodeExecError(value) {
    return typeof value === "object" && value !== null;
}
function escapeRegExp(value) {
    return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
function redactHookOutput(output, opts) {
    let redacted = output;
    const pathValues = [opts.cwd, homedir(), process.cwd()]
        .flatMap((pathValue) => pathValue.startsWith("/var/")
        ? [`/private${pathValue}`, pathValue]
        : [pathValue])
        .filter(Boolean);
    for (const pathValue of pathValues) {
        const isWorkspacePath = pathValue === opts.cwd || pathValue === `/private${opts.cwd}`;
        redacted = redacted.replace(new RegExp(escapeRegExp(pathValue), "g"), isWorkspacePath ? "[workspace]" : "[path]");
    }
    redacted = redacted.replace(/\/private\[workspace\]/g, "[workspace]");
    redacted = redacted.replace(/(?:\/Users|\/home|\/var\/folders|\/private\/tmp|\/tmp)\/[^\s'"`<>)]*/g, "[path]");
    redacted = redacted.replace(/\b([A-Z0-9_]*(?:TOKEN|SECRET|PASSWORD|PASS|API[_-]?KEY|AUTH|CREDENTIAL|PRIVATE|ACCESS[_-]?KEY|REFRESH[_-]?TOKEN|SESSION|COOKIE)[A-Z0-9_]*)\s*=\s*([^\s]+)/gi, "$1=[REDACTED]");
    const sensitiveEnvPattern = /(TOKEN|SECRET|PASSWORD|PASS|API[_-]?KEY|AUTH|CREDENTIAL|PRIVATE|ACCESS[_-]?KEY|REFRESH[_-]?TOKEN|SESSION|COOKIE)/i;
    ...
    for (const [key, value] of Object.entries(opts.env)) {
        if (key === "LINEAR_ISSUE_IDENTIFIER")
            continue;
        if (!value || value.length < 4)
            continue;
        sensitiveValues.add(value);
...
function truncateHookOutputTail(output) {
...
export class GitService {
     ...
     */
    getGitMetadataDirectoriesForWorkspace(workspace) {
        const candidateWorkingDirs = new Set([
            workspace.path,
     ...
     */
    async determineBaseBranch(issue, repository, baseBranchOverride) {
        // Priority 0: Explicit override from [repo=name#branch] syntax
        if (baseBranchOverride) {
            this.logger.info(`Using commit-ish override '${baseBranchOverride}' as base branch for ${issue.identifier} in repo ${repository.name}`);
            return {
        ...
        try {
            ...
            if (isGraphiteIssue) {
                ...
                if (blockingIssues.length > 0) {
                    ...
                    this.logger.info(`Issue ${issue.identifier} has graphite label and is blocked by ${blockingIssue.identifier}`);
                    const blockingRawBranchName = blockingIssue.branchName ||
                        `${blockingIssue.identifier}-${(blockingIssue.title ?? "")
                            .toLowerCase()
                            .replace(/\s+/g, "-")
                            .substring(0, 30)}`;
                    const blockingBranchName = this.sanitizeBranchName(blockingRawBranchName);
                    const blockingBranchExists = await this.branchExists(blockingBranchName, repository.repositoryPath);
                    if (blockingBranchExists) {
                        this.logger.info(`Using blocking issue branch '${blockingBranchName}' as base for Graphite-stacked issue ${issue.identifier}`);
                        return {
                    ...
                    this.logger.info(`Blocking issue branch '${blockingBranchName}' not found, falling back to parent/default`);
                }
            }
        }
        catch (_error) {
            this.logger.info(`Failed to check graphite label for ${issue.identifier}, falling back to parent/default`);
        }
     ...
     */
    async createGitWorktree(issue, repositories, options) {
        const { globalSetupScript, onRepoSetupHookEvent, workspaceBaseDir: overrideBaseDir, baseBranchOverrides, } = options ?? {};
        if (repositories.length === 0) {
            // 0 repos: create a plain folder (no git worktree)
            const baseDir = overrideBaseDir;
            if (!baseDir) {
                throw new Error("workspaceBaseDir is required in options when no repositories are provided");
            }
            const workspacePath = join(baseDir, issue.identifier);
            mkdirSync(workspacePath, { recursive: true });
            this.logger.info(`Created plain workspace (no repos) at ${workspacePath}`);
        ...
        if (repositories.length === 1) {
            ...
            this.logger.info(`createGitWorktree: baseBranchOverrides=${baseBranchOverrides ? `Map(size=${baseBranchOverrides.size})` : "undefined"}, repoId=${repoId}, overrideValue=${overrideValue ?? "undefined"}`);
            return this.createSingleRepoWorktree(issue, repositories[0], globalSetupScript, undefined, overrideValue, onRepoSetupHookEvent);
        }
        ...
        this.logger.info(`Creating multi-repo workspace at ${parentPath} for ${repositories.length} repositories`);
        // Run global setup script once in the parent directory
        if (globalSetupScript) {
            await this.runSetupScript(globalSetupScript, "global", parentPath, issue);
        }
        const repoPaths = {};
        const resolvedBaseBranches = {};
        for (const repository of repositories) {
            const repoSubPath = join(parentPath, repository.name);
            this.logger.info(`Creating worktree for repo '${repository.name}' at ${repoSubPath}`);
            try {
                const repoWorkspace = await this.createSingleRepoWorktree(issue, repository, undefined, // global setup already ran
                repoSubPath, // override workspace path for N-repo layout
                baseBranchOverrides?.get(repository.id), onRepoSetupHookEvent);
                repoPaths[repository.id] = repoWorkspace.path;
                if (repoWorkspace.resolvedBaseBranches) {
                    Object.assign(resolvedBaseBranches, repoWorkspace.resolvedBaseBranches);
                }
            }
            catch (error) {
                this.logger.error(`Failed to create worktree for repo '${repository.name}': ${error.message}`);
     ...
     */
    async createSingleRepoWorktree(issue, repository, globalSetupScript, workspacePathOverride, baseBranchOverride, onRepoSetupHookEvent) {
        this.logger.info(`createSingleRepoWorktree for ${repository.name} (id=${repository.id}): baseBranchOverride=${baseBranchOverride ?? "undefined"}`);
        // Build a fallback resolution for error paths where determineBaseBranch hasn't run
        const fallbackResolution = baseBranchOverride
            ? {
                branch: baseBranchOverride,
                source: "commit-ish",
                detail: `[repo=...#${baseBranchOverride}]`,
        ...
        try {
            ...
            mkdirSync(workspacePathOverride
                ? join(workspacePath, "..")
                : repository.workspaceBaseDir, { recursive: true });
            ...
            const resolution = await this.determineBaseBranch(issue, repository, baseBranchOverride);
            ...
            try {
                ...
                if (worktreeLines.includes(workspacePath)) {
                    ...
                    this.logger.info(`Stale worktree entry found for ${workspacePath}, pruning and recreating`);
                    try {
            ...
            await this.runRepoSetupScript(workspacePath, issue, repository.name, onRepoSetupHookEvent);
            return {
                path: workspacePath,
                isGitWorktree: true,
                resolvedBaseBranches: { [repository.id]: resolution },
            ...
            const fallbackPath = workspacePathOverride ??
                join(repository.workspaceBaseDir, issue.identifier);
     ...
     */
    async deleteWorktree(issueIdentifier, options = {}) {
        const workspacePath = join(getDefaultWorktreesDir(this.cyrusHome), issueIdentifier);
        if (!existsSync(workspacePath)) {
            this.logger.info(`Worktree directory does not exist for ${issueIdentifier}, nothing to delete`);
            return;
        }
        this.logger.info(`Deleting worktree directory for ${issueIdentifier} at ${workspacePath}`);
        ...
        for (const wtPath of worktreePaths) {
            try {
                ...
                this.logger.warn(`Failed to remove git worktree at ${wtPath}: ${error.message}`);
        ...
        try {
            ...
            this.logger.error(`Failed to delete worktree directory for ${issueIdentifier}: ${error.message}`);
        }
     ...
     */
    async runTeardownsForIssue(opts) {
        ...
        for (const workspacePath of targets) {
            try {
                ...
                this.logger.error(`Unexpected error running teardown for ${workspacePath}: ${error.message}`);
            }
        }
    }
    /**
     ...
     */
    findWorktreesUnderPath(dirPath) {
        const worktrees = [];
     ...
     */
    getMainRepoFromWorktree(worktreePath) {
        try {
     ...
     */
    async runRepoSetupScript(workspacePath, issue, repositoryName, onRepoSetupHookEvent) {
     ...
     */
    async runRepoTeardownScript(workspacePath, issueIdentifier) {
        await this.runRepoHookScript({
            hook: "teardown",
            workspacePath,
            env: {
    ...
    async runRepoHookScript(opts) {
        ...
        const candidates = [
            { file: `cyrus-${opts.hook}.sh`, platform: "unix" },
            { file: `cyrus-${opts.hook}.ps1`, platform: "windows" },
            { file: `cyrus-${opts.hook}.cmd`, platform: "windows" },
            { file: `cyrus-${opts.hook}.bat`, platform: "windows" },
        ];
        ...
        const fallback = !available && isWindows
            ? candidates.find((c) => {
                const scriptPath = join(opts.workspacePath, c.file);
    ...
    async emitRepoSetupHookEvent(handler, event) {
        if (!handler)
            return;
        try {
            await handler(event);
            ...
            this.logger.warn(`Failed to post repository setup hook activity: ${error.message}`);
        }
    }
    /**
     ...
     */
    async runHookScript(opts) {
        const { scriptPath, hook, originLabel, cwd, env, timeoutMs, repositoryName, issueIdentifier, onRepoSetupHookEvent, } = opts;
        ...
        const shouldPostRepoSetupActivity = Boolean(originLabel === "repository" &&
            hook === "setup" &&
            issueIdentifier &&
            onRepoSetupHookEvent);
        ...
        const runsThroughInterpreter = expandedPath.endsWith(".sh") || expandedPath.endsWith(".ps1");
        ...
        if (process.platform !== "win32" &&
            ...
            try {
                ...
                if (!(stats.mode & 0o100)) {
                    this.logger.warn(`⚠️  ${labelTitle} script is not executable: ${scriptPath}`);
                    ...
                    if (shouldPostRepoSetupActivity) {
                        await this.emitRepoSetupHookEvent(onRepoSetupHookEvent, {
                            ...
                            stderrTail: "Make cyrus-setup.sh executable in the repository and commit the executable bit: git update-index --chmod=+x cyrus-setup.sh",
                            truncated: false,
                        });
                    }
                    return;
                ...
                this.logger.warn(`⚠️  Cannot check permissions for ${labelTitle.toLowerCase()} script: ${error.message}`);
                return;
            }
        }
        this.logger.info(`ℹ️  Running ${labelTitle.toLowerCase()} script: ${scriptName}`);
        if (shouldPostRepoSetupActivity) {
            await this.emitRepoSetupHookEvent(onRepoSetupHookEvent, {
                status: "started",
                issueIdentifier: issueIdentifier,
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                ...
                child.on("close", (code, signal) => {
                    ...
                    error.outputTruncated = stdoutTail.truncated || stderrTail.truncated;
                    reject(error);
                });
            });
            this.logger.info(`✅ ${labelTitle} script completed successfully`);
            ...
            if (shouldPostRepoSetupActivity) {
                ...
                const stdoutTail = typeof nodeError.stdoutTail === "string"
                    ? nodeError.stdoutTail
                    : undefined;
                const stderrTail = typeof nodeError.stderrTail === "string"
