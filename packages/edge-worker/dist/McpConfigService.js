...
export class McpConfigService {
     ...
     */
    buildMcpConfig(repoId, linearWorkspaceId, parentSessionId) {
        ...
        const prebuiltServer = createCyrusToolsServer(linearClient, this.deps.createCyrusToolsOptions(parentSessionId));
     ...
     */
    buildMergedMcpConfigPath(repositories) {
     ...
     */
    isAuthorizationValid(rawAuthorizationHeader) {
    ...
    buildContextId(repoId, parentSessionId) {
        if (parentSessionId) {
            return `${repoId}:${parentSessionId}`;
        }
        return `${repoId}:anon:${Date.now()}:${Math.random().toString(36).slice(2, 8)}`;
    }
    pruneContexts(maxEntries = 500) {
        if (this.contexts.size <= maxEntries) {
            return;
        }
        const entriesByAge = Array.from(this.contexts.entries()).sort((a, b) => a[1].createdAt - b[1].createdAt);
