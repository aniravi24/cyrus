...
import { buildPromptText, SlackMessageService, SlackReactionService, } from "cyrus-slack-event-transport";
/**
...
export class SlackChatAdapter {
    ...
    async getSelfBotId(token) {
        ...
        try {
            ...
            this.logger.warn(`Failed to resolve bot identity: ${error instanceof Error ? error.message : String(error)}`);
            return undefined;
        }
    }
...
- If the user asks you to make repo code changes immediately, use these steps:
  ...
  - To choose a specific execution harness, add \`[agent=claude]\`, \`[agent=gemini]\`, \`[agent=codex]\`, \`[agent=cursor]\`, \`[agent=opencode]\`, or \`[agent=omp]\` to the issue description.
...
- Lists: use plain numbered lines (1. item) or dashes (- item) with newlines`;
    ...
    async notifyBusy(event) {
        await this.postNotice(event, "I'm still working on the previous request in this thread. I'll pick up your new message once I'm done.");
    }
    isSelfMessage(msg, selfBotId) {
        return Boolean(selfBotId && msg.bot_id === selfBotId);
    }
    formatThreadContext(messages, selfBotId) {
