...
export class PromptBuilder {
     ...
     */
    async determineSystemPromptFromLabels(labels, repositories) {
        if (labels.length === 0) {
            return undefined;
        }
        ...
        const anyRepoHasLabelPrompts = repositories.some((repo) => repo.labelPrompts);
        ...
        for (const repository of repositories) {
            ...
            const matchResult = await this.matchSystemPromptForRepo(lowercaseLabels, labels, repository, hasHardcodedOrchestratorLabel);
            if (!matchResult) {
                continue;
            }
            if (!winningResult) {
                ...
                this.logger.warn(`Label prompt conflict: repo '${repository.name}' would match '${matchResult.type}' ` +
     ...
     */
    async matchSystemPromptForRepo(lowercaseLabels, labels, repository, hasHardcodedOrchestratorLabel) {
        if (!repository.labelPrompts) {
            return undefined;
        }
        ...
        if (hasGraphiteLabel && hasOrchestratorLabel) {
            try {
                ...
                const promptContent = await readFile(promptPath, "utf-8");
                this.logger.debug(`Using graphite-orchestrator system prompt from repo '${repository.name}' for labels: ${labels.join(", ")}`);
                const promptVersion = this.extractVersionTag(promptContent);
                if (promptVersion) {
        ...
        for (const promptType of promptTypes) {
            ...
            if (matchesLabel) {
                try {
                    ...
                    const promptContent = await readFile(promptPath, "utf-8");
                    this.logger.debug(`Using ${promptType} system prompt from repo '${repository.name}' for labels: ${labels.join(", ")}`);
                    const promptVersion = this.extractVersionTag(promptContent);
                    if (promptVersion) {
     ...
     */
    async buildLabelBasedPrompt(issue, repositories, attachmentManifest = "", guidance, resolvedBaseBranches) {
        ...
        try {
            ...
            const baseBranchMap = await this.determineBaseBranch(issue, repositories, resolvedBaseBranches);
            const baseBranch = baseBranchMap.get(repository.id) ?? repository.baseBranch;
            ...
            const issueTracker = this.issueTrackers.get(requireLinearWorkspaceId(repository));
            if (!issueTracker) {
    ...
    generateRoutingContextForAllWorkspaces() {
        ...
        const workspaceIds = Array.from(activeRepositoriesByWorkspace.entries())
            ...
            .sort(([aWorkspaceId], [bWorkspaceId]) => aWorkspaceId.localeCompare(bWorkspaceId));
    ...
    generateRoutingContext(currentRepository) {
        ...
        const workspaceRepos = Array.from(this.repositories.values()).filter((repo) => repo.linearWorkspaceId ===
        ...
        const repoDescriptions = workspaceRepos.map((repo) => {
            ...
            const repoIdentifier = repo.githubUrl
                ...
                : repo.gitlabUrl
                    ...
                    : repo.name;
            routingMethods.push(`    - Description tag: \`[repo=${repoIdentifier}]\` or \`[repo=${repoIdentifier}#branch]\` for base branch override`);
            // Label-based routing
...
1. **Description Tag (Priority 1 - Highest, Recommended)**: Add \`[repo=repo-name]\` to the sub-issue description.
   ...
   - Agent selector: \`[agent=claude|gemini|codex|cursor|opencode|omp]\` to choose the execution harness
...
Focus on addressing the specific request in the mention. You can use the Linear MCP tools to fetch additional context if needed.`;
     ...
     * @returns Formatted prompt string
     */
    async buildIssueContextPrompt(issue, repositories, newComment, attachmentManifest = "", guidance, resolvedBaseBranches, workspaceRepoPaths) {
        const repository = repositories[0];
        ...
        try {
            ...
            let prompt = template
                ...
                .replace(/{{working_directory}}/g, workspaceRepoPaths?.[repository.id] ?? repository.repositoryPath)
            ...
            if (repositories.length > 1) {
                const repoSections = repositories
                    .map((repo) => {
                    const repoBranch = baseBranchMap.get(repo.id) ?? repo.baseBranch;
                    const workingDir = workspaceRepoPaths?.[repo.id] ?? repo.repositoryPath;
                    return `  <repository name="${repo.name}">\n    <working_directory>${workingDir}</working_directory>\n    <base_branch>${repoBranch}</base_branch>\n  </repository>`;
                })
                    .join("\n");
                prompt = prompt.replace(/<context>[\s\S]*?<\/context>/, `<repositories>\n${repoSections}\n</repositories>`);
            }
...
IMPORTANT: Focus specifically on addressing the new comment above. This is a new request that requires your attention.`;
            ...
            for (const repo of repositories) {
                if (repo.appendInstruction) {
                    this.logger.debug(`Adding repository-specific instruction for ${repo.name}`);
...
${reply.body}
     ...
     */
    async determineBaseBranch(issue, repositories, resolvedBaseBranches) {
        ...
        for (const repository of repositories) {
            const baseBranch = await this.determineBaseBranchForRepo(issue, repository, isGraphiteIssue, blockingIssues, parent);
     ...
     */
    async determineBaseBranchForRepo(issue, repository, isGraphiteIssue, blockingIssues, parent) {
        ...
        if (isGraphiteIssue && blockingIssues && blockingIssues.length > 0) {
            ...
            if (blockingBranchExists) {
                this.logger.debug(`Using blocking issue branch '${blockingBranchName}' as base for Graphite-stacked issue ${issue.identifier} in repo ${repository.name}`);
                return blockingBranchName;
            }
            this.logger.debug(`Blocking issue branch '${blockingBranchName}' not found in repo ${repository.name}, falling back to parent/default`);
        }
        if (parent) {
            ...
            if (parentBranchExists) {
                ...
                this.logger.debug(`Using parent issue branch '${parentBranchName}' as base for sub-issue ${issue.identifier} in repo ${repository.name}`);
            }
            else {
                this.logger.debug(`Parent branch '${parentBranchName}' not found in repo ${repository.name}, using default base branch '${repository.baseBranch}'`);
            }
        }
        else {
            this.logger.debug(`No parent issue found for ${issue.identifier}, using default base branch '${repository.baseBranch}' for repo ${repository.name}`);
        }
        return baseBranch;
    }
    /**
     ...
     */
    async hasGraphiteLabel(issue, repositories) {
        ...
        for (const repository of repositories) {
            ...
            const graphiteLabels = Array.isArray(graphiteConfig)
                ? graphiteConfig
                : (graphiteConfig?.labels ?? ["graphite"]);
            if (graphiteLabels.some((label) => issueLabels.includes(label))) {
                return true;
            }
        }
        return false;
