...
export class EdgeWorker extends EventEmitter {
        ...
        const repositoryRouterDeps = {
            ...
            fetchIssueDescription: async (issueId, workspaceId) => {
                ...
                try {
                    ...
                    console.error(`[EdgeWorker] Failed to fetch issue description for routing:`, error);
                    return undefined;
                }
            },
    ...
    async handleResumeParentSession(parentSessionId, prompt, childSessionId, _childRepo, childAgentSessionManager) {
        ...
        console.log(`[Parent Session Resume] Searching for parent session ${parentSessionId} across all repositories`);
        let parentSession;
        let parentRepo;
        ...
        for (const [repoId, manager] of this.agentSessionManagers) {
            ...
            if (candidate) {
                ...
                console.log(`[Parent Session Resume] Found parent session in repository: ${parentRepo?.name || repoId}`);
                break;
            }
        }
        if (!parentSession || !parentRepo || !parentAgentSessionManager) {
            console.error(`[Parent Session Resume] Parent session ${parentSessionId} not found in any repository's agent session manager`);
            return;
        }
        ...
        try {
            ...
            console.error(`[Parent Session Resume] Error context - Parent issue: ${parentSession.issueId}, Repository: ${parentRepo.name}`);
        }
    }
    /**
     ...
     */
    async createLinearAgentSession(linearAgentActivitySessionId, issue, repository, agentSessionManager) {
        ...
        if (!session) {
            throw new Error(`Failed to create session for agent activity session ${linearAgentActivitySessionId}`);
        }
    ...
    async initializeAgentRunner(agentSession, repository, guidance, commentBody) {
        ...
        try {
            ...
            if (currentSubroutine?.disallowAllTools) {
                console.log(`[EdgeWorker] All tools disabled for ${fullIssue.identifier} (subroutine: ${currentSubroutine.name})`);
            }
            else {
    ...
    async determineSystemPromptFromLabels(labels, repository) {
        ...
        if (!repository.labelPrompts && hasHardcodedOrchestratorLabel) {
            try {
                ...
                console.log(`[EdgeWorker] Using orchestrator system prompt (hardcoded rule) for labels: ${labels.join(", ")}`);
                const promptVersion = this.extractVersionTag(promptContent);
                if (promptVersion) {
                    console.log(`[EdgeWorker] orchestrator system prompt version: ${promptVersion}`);
                }
                return {
                ...
                console.error(`[EdgeWorker] Failed to load orchestrator prompt template:`, error);
                return undefined;
            }
        }
...
Focus on addressing the specific request in the mention. You can use the Linear MCP tools to fetch additional context if needed.`;
    ...
    async determineBaseBranch(issue, repository) {
        ...
        try {
            ...
            if (parent) {
                console.log(`[EdgeWorker] Issue ${issue.identifier} has parent: ${parent.identifier}`);
                ...
                if (parentBranchExists) {
                    ...
                    console.log(`[EdgeWorker] Parent branch '${parentBranchName}' not found, using default base branch '${repository.baseBranch}'`);
                }
            }
        }
        catch (_error) {
            // Parent field might not exist or couldn't be fetched, use default base branch
            console.log(`[EdgeWorker] No parent issue found for ${issue.identifier}, using default base branch '${repository.baseBranch}'`);
        }
        return baseBranch;
    }
    /**
            ...
            if (issueTracker && issue.id) {
                try {
                    ...
                    if (attachments?.nodes) {
                        ...
                        console.log(`[EdgeWorker] Found ${nativeAttachments.length} native attachments`);
                    }
                }
                catch (error) {
...
${input.userComment}
    ...
    async resumeInterruptedSessions() {
        ...
        for (const [repositoryId, agentSessionManager,] of this.agentSessionManagers.entries()) {
            ...
            if (!repository) {
                console.warn(`[EdgeWorker] Repository ${repositoryId} not found for interrupted sessions`);
                continue;
            }
            ...
            for (const session of interruptedSessions) {
                try {
                    ...
                    console.log(`[EdgeWorker] Resuming interrupted session ${session.linearAgentActivitySessionId} for issue ${session.issue?.identifier || session.issueId}`);
                    // Post a thought activity to inform the user
                    await agentSessionManager.createThoughtActivity(session.linearAgentActivitySessionId, "Resuming after interruption. Let me continue where I left off.");
                    // Resume the session with the recovery prompt
                    await this.resumeAgentSession(session, repository, session.linearAgentActivitySessionId, agentSessionManager, recoveryPrompt, "", // No attachment manifest for recovery
                    true);
                    resumedCount++;
                }
                catch (error) {
                    console.error(`[EdgeWorker] Failed to resume interrupted session ${session.linearAgentActivitySessionId}:`, error);
    ...
    async resumeAgentSession(session, repository, linearAgentActivitySessionId, agentSessionManager, promptBody, attachmentManifest = "", isNewSession = false, additionalAllowedDirectories = [], maxTurns, commentAuthor, commentTimestamp) {
        ...
        if (currentSubroutine?.disallowAllTools) {
            console.log(`[resumeClaudeSession] All tools disabled for subroutine: ${currentSubroutine.name}`);
        }
        ...
        if (resumeSessionId && runnerType === "claude") {
            runner.on("resume-failed", async (staleSessionId) => {
                console.log(`[resumeAgentSession] Resume failed for session ${linearAgentActivitySessionId}, stale ID: ${staleSessionId}`);
                ...
                await agentSessionManager.createThoughtActivity(linearAgentActivitySessionId, "Session interrupted - reconstructing context and continuing...");
                // Retry with a fresh session (recursively call resumeAgentSession with isNewSession=true)
                try {
                    await this.resumeAgentSession(session, repository, linearAgentActivitySessionId, agentSessionManager, recoveryPrompt, attachmentManifest, true, // Force new session (no resumeSessionId)
                    additionalAllowedDirectories, maxTurns, commentAuthor, commentTimestamp);
                    console.log(`[resumeAgentSession] Successfully recovered session ${linearAgentActivitySessionId} with fresh context`);
                }
                catch (retryError) {
                    console.error(`[resumeAgentSession] Failed to recover session ${linearAgentActivitySessionId}:`, retryError);
                    await agentSessionManager.createErrorActivity(linearAgentActivitySessionId, "Failed to recover session after interruption. Please try again.");
                }
            });
        }
        // Store runner
    ...
    buildOAuthConfig(repo) {
        ...
        if (!clientId || !clientSecret) {
            console.warn("[EdgeWorker] LINEAR_CLIENT_ID and LINEAR_CLIENT_SECRET not set, token refresh disabled");
            return undefined;
        }
        if (!repo.linearRefreshToken) {
            console.warn(`[EdgeWorker] No refresh token for repository ${repo.id}, token refresh disabled`);
            return undefined;
        }
    ...
    async saveOAuthTokens(tokens) {
        ...
        try {
            ...
            console.log(`[EdgeWorker] OAuth tokens saved to config for workspace ${tokens.linearWorkspaceId}`);
        }
        catch (error) {
