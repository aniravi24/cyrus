...
import { ConfigUpdater, ensureGhTokenResolver, ensureGitHubCredentialHelper, } from "cyrus-config-updater";
import { CLIIssueTrackerService, CLIRPCServer, createLogger, DEFAULT_PROXY_URL, GitHubTokenStore, isAgentSessionCreatedWebhook, isAgentSessionPromptedWebhook, isContentUpdateMessage, isIssueAssignedWebhook, isIssueCommentMentionWebhook, isIssueDeletedWebhook, isIssueNewCommentWebhook, isIssueStateChangeMessage, isIssueStateChangeWebhook, isIssueStateIdUpdateWebhook, isIssueTitleOrDescriptionUpdateWebhook, isIssueUnassignedWebhook, isSessionStartMessage, isStopSignalMessage, isUnassignMessage, isUserPromptMessage, PersistenceManager, requireLinearWorkspaceId, resolvePath, WebhookIpValidator, } from "cyrus-core";
...
import { ZulipEventTransport, } from "cyrus-zulip-event-transport";
...
import { RunnerConfigBuilder, resolveIssueMcpConfigPath, } from "./RunnerConfigBuilder.js";
 ...
 */
const GITHUB_REVIEW_REQUEST_PATTERN = /<!-- bot-review-(?:kickoff|rescue) sha=([0-9a-f]{7,40}) -->/;
export function extractReviewRequestSha(body) {
    return GITHUB_REVIEW_REQUEST_PATTERN.exec(body)?.[1] ?? null;
}
/**
 * Posted when maintenance mode is on and no `message` is configured.
 */
export const DEFAULT_MAINTENANCE_MESSAGE = "Cyrus is temporarily off for maintenance, so nothing was started for this request. Mention me again once it is back on.";
/**
 ...
 */
export function extractLeadingRunnerSelectors(commentBody, botUsername) {
    const handle = botUsername ? `@${botUsername}` : "@[\\w-]+";
    const match = commentBody.match(new RegExp(`^\\s*${handle}\\s*((?:\\[(?:model|agent)=[^\\]]+\\]\\s*)+)`, "i"));
...
export class EdgeWorker extends EventEmitter {
     ...
     */
    static normalizeConfigPaths(config) {
        const resolveList = (paths) => paths ? paths.map(resolvePath) : undefined;
        return {
            ...config,
    ...
    constructor(config) {
        ...
        const repositoryRouterDeps = {
            ...
            fetchIssueDescription: async (issueId, linearWorkspaceId) => {
                // Use workspace ID directly from webhook context (Linear-native source)
                const issueTracker = this.issueTrackers.get(linearWorkspaceId);
                if (!issueTracker)
                    return undefined;
            ...
            hasActiveSession: (issueId, _repositoryId) => {
                const activeSessions = this.agentSessionManager.getActiveSessionsByIssueId(issueId);
                return activeSessions.length > 0;
            },
            getIssueTracker: (linearWorkspaceId) => {
        ...
        this.agentSessionManager = new AgentSessionManager((childSessionId) => {
        ...
        if (config.linearWorkspaces) {
            for (const [linearWorkspaceId, wsConfig] of Object.entries(config.linearWorkspaces)) {
        ...
        this.attachmentService = new AttachmentService(this.logger, this.cyrusHome, this.config.linearWorkspaces || {});
        this.runnerSelectionService = new RunnerSelectionService(this.config);
        this.runnerSlots = new SessionSemaphore(this.config.maxConcurrentSessions ?? Number.POSITIVE_INFINITY, (message) => this.logger.info(message));
        this.toolPermissionResolver = new ToolPermissionResolver(this.config, this.logger);
        this.mcpConfigService = new McpConfigService({
            getLinearTokenForWorkspace: (workspaceId) => this.getLinearTokenForWorkspace(workspaceId),
            getIssueTracker: (workspaceId) => this.issueTrackers.get(workspaceId),
            getCyrusToolsMcpUrl: () => this.getCyrusToolsMcpUrl(),
            createCyrusToolsOptions: (parentSessionId) => this.createCyrusToolsOptions(parentSessionId),
        });
        this.runnerConfigBuilder = new RunnerConfigBuilder(this.toolPermissionResolver, this.mcpConfigService, this.runnerSelectionService);
    ...
    async start() {
        ...
        if (existsSync(this.githubTokenStore.filePath)) {
            try {
                ...
                this.logger.info("✅ GitHub auth scripts configured from existing token store");
            }
            catch (error) {
                this.logger.warn("Failed to configure GitHub auth scripts on startup (non-fatal):", error instanceof Error ? error : new Error(String(error)));
            }
        }
        ...
        this.configManager.on("configChanged", async (changes) => {
            const strictMcpConfigChanged = (this.config.strictMcpConfig ?? true) !==
            ...
            this.runnerSlots.setLimit(changes.newConfig.maxConcurrentSessions ?? Number.POSITIVE_INFINITY);
        });
    ...
    async initializeComponents() {
        ...
        if (this.config.platform === "cli") {
            ...
            if (firstCliTracker) {
                ...
                cliEventTransport.on("message", (message) => {
        ...
        this.logger.info("           /api/update/repository, /api/update/test-mcp, /api/update/configure-mcp");
    ...
    registerGitHubEventTransport() {
        ...
        const isExternalHost = process.env.CYRUS_HOST_EXTERNAL?.toLowerCase().trim() === "true";
        const hasGithubWebhookSecret = process.env.GITHUB_WEBHOOK_SECRET != null &&
        ...
        this.gitHubEventTransport.on("event", (event) => {
            ...
            if (event.eventType === "pull_request") {
                this.handleGitHubPullRequestClosed(event.payload).catch((error) => {
                    this.logger.error("Failed to handle closed GitHub pull request", error instanceof Error ? error : new Error(String(error)));
                });
                return;
            }
    ...
    getMaintenanceMode() {
        const envValue = (process.env.CYRUS_MAINTENANCE_MODE ?? "")
            .toLowerCase()
            .trim();
        const forcedOn = envValue === "true" || envValue === "1" || envValue === "yes";
        const configured = this.config.maintenanceMode;
        return {
            enabled: forcedOn || configured?.enabled === true,
            message: process.env.CYRUS_MAINTENANCE_MESSAGE?.trim() ||
     ...
     */
    buildChatSessionHandlerDeps(chatRepositoryProvider, getPlatformMcpConfigOverrides) {
        return {
            cyrusHome: this.cyrusHome,
            chatRepositoryProvider,
            runnerConfigBuilder: this.runnerConfigBuilder,
            createRunner: (config, chatRunnerType) => {
                const runnerType = chatRunnerType ?? this.runnerSelectionService.getDefaultRunner();
            ...
            resolveSkillsConfig: async ({ repository, repositoryPaths }) => {
                ...
                const skills = await this.skillsPluginResolver.discoverSkillNames(plugins, {
     ...
     */
    get activeChatSessionHandlers() {
        return [this.chatSessionHandler, this.zulipChatSessionHandler].filter((handler) => handler !== null);
    }
    /**
    ...
    registerZulipEventTransport() {
        ...
        const chatRepositoryProvider = new LiveChatRepositoryProvider(this.repositories, () => this.config.linearWorkspaces || {});
        const zulipAdapter = new ZulipChatAdapter(chatRepositoryProvider, this.logger, {
            repositoryRoutingContext: this.promptBuilder.generateRoutingContextForAllWorkspaces(),
        });
        this.zulipChatSessionHandler = new ChatSessionHandler(zulipAdapter, this.buildChatSessionHandlerDeps(chatRepositoryProvider, () => this.config.zulipMcpConfigs), this.logger);
        this.zulipEventTransport = new ZulipEventTransport({
            fastifyServer: this.sharedApplicationServer.getFastifyInstance(),
            token,
            credentials: { site, botEmail, apiKey },
        }, this.logger);
        this.zulipEventTransport.on("event", (event) => {
            this.zulipChatSessionHandler.handleEvent(event).catch((error) => {
                this.logger.error("Failed to handle Zulip webhook", error instanceof Error ? error : new Error(String(error)));
            });
        });
        this.zulipEventTransport.on("error", (error) => {
            this.handleError(error);
        });
    ...
    registerSlackEventTransport() {
        ...
        const cyrusAppBaseUrl = process.env.CYRUS_API_KEY
            ? getCyrusAppUrl()
            : undefined;
        const slackAdapter = new SlackChatAdapter(chatRepositoryProvider, this.logger, { repositoryRoutingContext: routingContext, cyrusAppBaseUrl });
        if (!chatRepositoryProvider.getDefaultLinearWorkspaceId() ||
        ...
        this.chatSessionHandler = new ChatSessionHandler(slackAdapter, this.buildChatSessionHandlerDeps(chatRepositoryProvider, 
        // Live read so hot-reloaded config (`setConfig`) picks up new
        // per-platform MCP paths without rebuilding the handler.
        () => this.config.slackMcpConfigs), this.logger);
        ...
        const isExternalHost = process.env.CYRUS_HOST_EXTERNAL?.toLowerCase().trim() === "true";
        const hasSlackSigningSecret = process.env.SLACK_SIGNING_SECRET != null &&
        ...
        this.logger.info(`Slack event transport registered (${slackVerificationMode} mode)`);
    }
    /**
     ...
     */
    async resolveGitHubToken(event, repository) {
        if (repository?.githubUrl) {
            const storedToken = this.githubTokenStore.getTokenForRepoUrl(repository.githubUrl);
            if (storedToken)
                return storedToken;
        }
    ...
    async handleGitHubWebhook(event, reservedGitHubPrSlot = false) {
        this.activeWebhookCount++;
        let githubPrQueueKey;
        ...
        try {
            ...
            if (botUsername && commentAuthor === botUsername) {
                this.logger.debug(`Ignoring comment from bot user @${botUsername} on ${repoFullName}#${prNumber}`);
                return;
            }
            ...
            if (isPullRequestReviewPayload(event.payload)) {
                if (event.payload.review.state !== "changes_requested") {
                    this.logger.debug(`Ignoring pull_request_review with state: ${event.payload.review.state}`);
                    return;
                }
            }
            ...
            if (isPullRequestReview && this.config.prReviewTrigger === false) {
                this.logger.debug(`PR review trigger is disabled, ignoring pull_request_review on ${repoFullName}#${prNumber}`);
                return;
            }
            ...
            if (!isPullRequestReview &&
                ...
                this.logger.debug(`Ignoring comment without @${botUsername} mention on ${repoFullName}#${prNumber}`);
                return;
            }
            ...
            if (maintenance.enabled) {
                this.logger.info(`Maintenance mode is on; declining GitHub webhook for ${repoFullName}#${prNumber}`);
                const token = await this.resolveGitHubToken(event, this.findRepositoryByGitHubUrl(repoFullName) ?? undefined);
                if (token && prNumber) {
                    await this.gitHubCommentService
                        ...
                        .catch((err) => {
                        this.logger.warn(`Failed to post maintenance notice: ${err instanceof Error ? err.message : err}`);
                    });
                }
                return;
            }
            ...
            const reactionToken = await this.resolveGitHubToken(event, this.findRepositoryByGitHubUrl(repoFullName) ?? undefined);
            ...
            if (!repository) {
                ...
                const wasMentioned = !!botUsername && commentBody.includes(`@${botUsername}`);
                ...
                if (shouldReply && reactionToken && prNumber) {
                    ...
                    this.gitHubCommentService
                        ...
                        .catch((err) => {
                        this.logger.warn(`Failed to post unconfigured-repo notice: ${err instanceof Error ? err.message : err}`);
                    });
                }
                return;
            }
            ...
            if (!reservedGitHubPrSlot) {
                if (this.activeGitHubPrSessions.has(sessionKey)) {
                    const stoppedStaleReview = this.enqueueGitHubPrEvent(sessionKey, event);
                    const waiting = this.queuedGitHubPrEvents.get(sessionKey)?.length ?? 0;
                    this.logger.info(`Queued GitHub webhook for ${repoFullName}#${prNumber}; ${waiting} event(s) waiting`);
                    if (!stoppedStaleReview && reactionToken && prNumber) {
                        this.gitHubCommentService
                            ...
                            .catch((err) => {
                            this.logger.warn(`Failed to post queued acknowledgement: ${err instanceof Error ? err.message : err}`);
                        });
                    }
                    return;
                }
            ...
            if (isPullRequestReview && reactionToken && prNumber) {
                this.gitHubCommentService
                    ...
                    .catch((err) => {
                    this.logger.warn(`Failed to post acknowledgement comment: ${err instanceof Error ? err.message : err}`);
                });
            }
            ...
            if (!branchRef || !prNumber) {
                this.logger.error(`Could not determine branch or PR number for ${repoFullName}#${prNumber}`);
                return;
            }
            ...
            if (multiRepoSession) {
                ...
                workspace = await this.createGitHubWorkspace(repository, branchRef, prNumber);
            }
            if (!workspace) {
            ...
            agentSessionManager.createCyrusAgentSession(githubSessionId, sessionKey, issueMinimal, workspace, "github", // Don't stream activities to Linear for GitHub sources
            [
                {
                    repositoryId: repository.id,
                    branchName: branchRef,
            ...
            this.buildSkillSessionContext(repository, undefined, session), "github");
                ...
                try {
                    ...
                    if (message.type === "result" && !githubReplyPosted) {
                        ...
                        this.postGitHubReply(event, runner, repository).catch((error) => {
                            this.logger.error(`Failed to post GitHub reply to ${repoFullName}#${prNumber}`, error instanceof Error ? error : new Error(String(error)));
                        });
    ...
    advanceGitHubPrQueue(sessionKey) {
        ...
        if (nextEvent) {
            ...
            this.handleGitHubWebhook(nextEvent, true).catch((error) => {
                this.logger.error("Failed to process queued GitHub webhook", error instanceof Error ? error : new Error(String(error)));
            });
        }
        else {
     ...
     */
    enqueueGitHubPrEvent(sessionKey, event) {
        const queue = this.queuedGitHubPrEvents.get(sessionKey) ?? [];
        ...
        const kept = queue.filter((queued) => !extractReviewRequestSha(extractCommentBody(queued)));
        kept.push(event);
        ...
        const activeSha = active
            ...
            : null;
        if (!active || !activeSha || activeSha === requestedSha)
            return false;
        ...
        if (live.length === 0)
            return false;
        this.logger.info(`Review of ${activeSha} on ${sessionKey} superseded by ${requestedSha}; stopping ${live.length} session(s)`);
     ...
     */
    async handleGitHubPullRequestClosed(payload) {
        ...
        if (live.length === 0)
            return;
        this.logger.info(`PR ${sessionKey} closed (${payload.pull_request.merged ? "merged" : "unmerged"}); stopping ${live.length} session(s)`);
        for (const session of live) {
            this.agentSessionManager.requestSessionStop(session.id);
            session.agentRunner?.stop();
        }
...
Your base branch \`${branchName}\` has received ${commitCount} new commit(s). Consider rebasing your working branch onto the updated base to avoid merge conflicts. You can do this with: \`git fetch origin && git rebase origin/${branchName}\`
        ...
        for (const session of sortedSessions) {
            ...
            if (isRunning &&
                ...
                try {
                    ...
                    this.logger.debug(`[base-branch-update] Stream rejected for session ${session.id}; skipping`, { error: error instanceof Error ? error.message : String(error) });
                }
            }
        }
    }
     ...
     */
    async fetchPRBranchRefs(event, repository) {
        ...
        try {
            ...
            this.logger.error("Failed to fetch PR branch refs", error instanceof Error ? error : new Error(String(error)));
            return null;
        }
    }
    /**
...
- Be concise in your responses as they will be posted back to the GitHub PR`;
     ...
     */
    buildGitHubChangeRequestSystemPrompt(event, branchRef, reviewBody) {
        const repoFullName = extractRepoFullName(event);
        const prNumber = extractPRNumber(event);
        const prTitle = extractPRTitle(event);
        const commentAuthor = extractCommentAuthor(event);
...
${taskSection}`;
     ...
     */
    async postGitHubReply(event, runner, repository) {
        try {
            ...
            if (this.supersededGitHubDeliveries.has(event.deliveryId)) {
                this.logger.info("Skipping GitHub reply: session was superseded by a newer review");
                return;
            }
            ...
            if (!summary) {
                this.logger.info("Skipping GitHub reply: session produced no final text block");
                return;
            }
            ...
            if (summary.trimStart().startsWith(GITHUB_NO_REPLY_MARKER)) {
                this.logger.info(`Skipping GitHub reply: final text opted out with ${GITHUB_NO_REPLY_MARKER}`);
                return;
            }
            const owner = extractRepoOwner(event);
            const repo = extractRepoName(event);
    ...
    async handleGitLabWebhook(event) {
        ...
        try {
            ...
            200, // maxTurns
            undefined, // linearWorkspaceId
            this.buildSkillSessionContext(repository, undefined, session), "gitlab");
...
${taskSection}`;
    ...
    computeStatus() {
        ...
        if (this.activeChatSessionHandlers.some((handler) => handler.isAnyRunnerBusy())) {
            return "busy";
        }
        return "idle";
    }
    ...
    async handleResumeParentSession(parentSessionId, prompt, childSessionId) {
        ...
        await this.postParentResumeAcknowledgment(parentSessionId, parentWorkspaceId);
        ...
        try {
            ...
            "parent resume from child", parentWorkspaceId);
    ...
    async removeDeletedRepositories(repos) {
        for (const repo of repos) {
            try {
                ...
                const activeSessions = allActiveSessions.filter((s) => this.sessionRepositories.get(s.id) === repo.id);
                if (activeSessions.length > 0) {
                    ...
                    for (const session of activeSessions) {
                        try {
                            ...
                            const runner = this.agentSessionManager.getAgentRunner(session.id);
                            if (runner) {
     ...
     */
    getCachedRepositories(issueId) {
        return this.repositoryRouter.getCachedRepositories(issueId, this.repositories);
    }
    /**
     ...
     */
    async handleIssueStateChangeMessage(message) {
        this.logger.info(`[MessageBus] Issue reached terminal state: ${message.workItemIdentifier}`);
        ...
        for (const session of sessions) {
            this.logger.info(`Stopping agent runner for ${message.workItemIdentifier} (issue terminal)`);
            this.agentSessionManager.requestSessionStop(session.id);
            session.agentRunner?.stop();
        }
        ...
        for (const session of sessions) {
            await this.agentSessionManager.createResponseActivity(session.id, `Session stopped — ${message.workItemIdentifier} was marked as Done or Canceled.`);
        ...
        const teardownRepositories = [];
        for (const repoId of repoIds) {
            const repo = this.repositories.get(repoId);
            if (repo)
                teardownRepositories.push(repo);
        }
        ...
        this.logger.info(`Completed cleanup for ${message.workItemIdentifier}: stopped ${sessions.length} session(s)`);
    }
    // ============================================================================
    ...
    async handleIssueUnassignedWebhook(webhook) {
        ...
        if (!repository) {
            ...
            this.logger.info(`No cached repository for issue unassignment ${webhook.notification.issue.identifier}, searching sessions`);
            const sessions = this.agentSessionManager.getSessionsByIssueId(issueId);
            if (sessions.length > 0) {
                ...
                if (repoId) {
                    ...
                    if (repository) {
                        this.logger.info(`Recovered repository ${repoId} for unassignment of ${webhook.notification.issue.identifier} from session manager`);
                    }
                }
                if (!repository) {
                    // Sessions exist but no repository mapping — still stop the sessions
                    this.logger.warn(`Found ${sessions.length} session(s) for unassigned issue ${webhook.notification.issue.identifier} but no repository mapping, stopping sessions without farewell comment`);
                    for (const session of sessions) {
                        this.agentSessionManager.requestSessionStop(session.id);
                        session.agentRunner?.stop();
                    }
            ...
            if (!repository) {
                this.logger.debug(`No active sessions found for unassigned issue ${webhook.notification.issue.identifier}`);
                return;
            }
        }
        this.logger.info(`Handling issue unassignment: ${webhook.notification.issue.identifier}`);
        await this.handleIssueUnassigned(webhook.notification.issue, webhook.organizationId);
    }
    /**
    ...
    async handleIssueContentUpdate(webhook) {
        ...
        if (this.processedIssueUpdateKeys.has(webhookKey)) {
            this.logger.debug(`Duplicate issue update webhook for ${issueIdentifier} (key=${webhookKey}), skipping`);
            return;
        }
        ...
        if (!repository) {
            // Fallback: search sessions for this issue to find the repository
            const issueSessions = this.agentSessionManager.getSessionsByIssueId(issueId);
            if (issueSessions.length > 0) {
                ...
                if (repoId) {
                    repository = this.repositories.get(repoId) ?? null;
                    if (repository) {
                        this.logger.info(`Recovered repository ${repoId} for issue update ${issueIdentifier} from session manager`);
                    }
                }
            }
            if (!repository) {
                this.logger.debug(`No active sessions found for issue update ${issueIdentifier}`);
                return;
            }
        }
        ...
        if ("description" in updatedFrom && issueData.description) {
            ...
            try {
                ...
                const linearToken = this.getLinearTokenForWorkspace(webhook.organizationId);
                const downloadResult = await this.downloadCommentAttachments(issueData.description, attachmentsDir, linearToken, existingAttachmentCount);
        ...
        for (const session of sortedSessions) {
            ...
            if (isRunning &&
                ...
                try {
                    ...
                    delivered = true;
                    this.logger.debug(`[issue-update] Streamed update to session ${sessionId} (key=${webhookKey}, changed=[${changedFields.join(", ")}])`);
                    break;
                }
                catch (error) {
                    this.logger.debug(`[issue-update] Stream rejected for session ${sessionId}; skipping (key=${webhookKey})`, { error: error instanceof Error ? error.message : String(error) });
                }
            }
            else if (isRunning) {
                this.logger.debug(`[issue-update] Session ${sessionId} is running but doesn't support streaming input, skipping (key=${webhookKey})`);
            }
            else {
                this.logger.debug(`[issue-update] Session ${sessionId} is idle, ignoring update (key=${webhookKey})`);
            }
        }
        if (!delivered) {
            this.logger.debug(`[issue-update] No running streaming sessions for ${issueIdentifier}, update discarded (key=${webhookKey})`);
        }
    }
    /**
     ...
     */
    async createCyrusAgentSession(sessionId, issue, repositoriesOrSingle, agentSessionManager, linearWorkspaceId, baseBranchOverrides, routingMethod) {
        ...
        this.logger.info(`createCyrusAgentSession: passing baseBranchOverrides=${baseBranchOverrides ? `Map(size=${baseBranchOverrides.size}, keys=[${Array.from(baseBranchOverrides.keys()).join(",")}])` : "undefined"}, useCustomHandler=${!!this.config.handlers?.createWorkspace}`);
        const workspace = this.config.handlers?.createWorkspace
            ? await this.config.handlers.createWorkspace(fullIssue, repositories, {
                baseBranchOverrides,
                onRepoSetupHookEvent: (activity) => this.activityPoster.postRepoSetupHookActivity(sessionId, linearWorkspaceId, activity),
            })
        ...
        const repositoryContexts = repositories.map((repo) => ({
            ...
            baseBranchName: workspace.resolvedBaseBranches?.[repo.id]?.branch ?? repo.baseBranch,
        }));
        ...
        // Post combined routing + base branch activity
        {
            ...
            await this.postRoutingActivity(sessionId, linearWorkspaceId, repoLines, routingMethod);
        }
        ...
        const attachmentResult = await this.downloadIssueAttachments(fullIssue, linearWorkspaceId, workspace.path);
        ...
        await writeFile(join(claudeSettingsDir, "settings.local.json"), JSON.stringify({
    ...
    async handleAgentSessionCreatedWebhook(webhook, repos) {
        ...
        let baseBranchOverrides;
        let routingMethod;
        if (issueId) {
            const cachedRepos = this.getCachedRepositories(issueId);
            if (cachedRepos && cachedRepos.length > 0) {
                repositories = cachedRepos;
                this.logger.debug(`Using cached repositories [${cachedRepos.map((r) => r.name).join(", ")}] for issue ${issueId}`);
            }
        }
        ...
        if (!repositories) {
            ...
            if (baseBranchOverrides && baseBranchOverrides.size > 0) {
                this.logger.info(`baseBranchOverrides received from routing: ${Array.from(baseBranchOverrides.entries())
            ...
            if (issueId) {
                this.repositoryRouter.getIssueRepositoryCache().set(issueId, repositories.map((r) => r.id));
            }
        }
        ...
        const maintenance = this.getMaintenanceMode();
        if (maintenance.enabled) {
            this.logger.info(`Maintenance mode is on; declining Linear session ${webhook.agentSession.id}`);
        ...
        await this.linkChildSessionToParentIssueSession(agentSession.id, agentSession.issue, linearWorkspaceId);
     ...
     */
    async initializeAgentRunner(agentSession, repositories, linearWorkspaceId, guidance, commentBody, baseBranchOverrides, routingMethod) {
        ...
        try {
            ...
            if (!isMentionTriggered || isLabelBasedPromptRequested) {
                const systemPromptResult = await this.determineSystemPromptFromLabels(labels, primaryRepo);
            ...
            undefined, // maxTurns
            linearWorkspaceId, this.buildSkillSessionContext(primaryRepo, fullIssue, session));
            ...
            this.attachCrashRecoveryHandler(runner, primaryRepo, sessionId, agentSessionManager);
            ...
            this.config.handlers?.onSessionStart?.(fullIssue.id, fullIssue, primaryRepo.id);
    ...
    async handleStopSignal(webhook) {
        ...
        if (!foundSession) {
            ...
            log.info(`No session found for stop signal ${agentSessionId} (likely a legacy session after restart)`);
    ...
    async handleRepositorySelectionResponse(webhook) {
        ...
        await this.linkChildSessionToParentIssueSession(agentSessionId, agentSession.issue, webhook.organizationId);
        ...
        // Use organizationId from webhook as the Linear-native workspace ID source
        await this.initializeAgentRunner(agentSession, [repository], webhook.organizationId, guidance, commentBody, undefined, "user-selected");
    }
    /**
     ...
     */
    async handleNormalPromptedActivity(webhook, repositories) {
        ...
        if (!session) {
            ...
            await this.postInstantPromptedAcknowledgment(sessionId, linearWorkspaceId, false);
            // Create the session using the shared method with all repositories
            const sessionData = await this.createCyrusAgentSession(sessionId, issue, repositories, agentSessionManager, linearWorkspaceId);
            ...
            await this.postInstantPromptedAcknowledgment(sessionId, linearWorkspaceId, isCurrentlyStreaming);
        ...
        const issueTracker = this.issueTrackers.get(linearWorkspaceId);
        if (!issueTracker) {
            this.logger.error("Unexpected: There was no IssueTrackerService for workspace", linearWorkspaceId);
            return;
        }
        ...
        try {
            ...
            const linearTokenForAttachments = this.getLinearTokenForWorkspace(linearWorkspaceId);
            const downloadResult = comment
                ? await this.downloadCommentAttachments(comment.body, attachmentsDir, linearTokenForAttachments, existingAttachmentCount)
                : {
        ...
        try {
            ...
            `prompted webhook (${isNewSession ? "new" : "existing"} session)`, linearWorkspaceId, commentAuthor, commentTimestamp);
        }
        catch (error) {
    ...
    async handleUserPromptedAgentActivity(webhook) {
        ...
        const maintenance = this.getMaintenanceMode();
        if (maintenance.enabled) {
            this.logger.info(`Maintenance mode is on; declining prompt on Linear session ${agentSessionId}`);
            await this.postMaintenanceNotice(webhook, maintenance.message);
            return;
        }
        ...
        if (!repositories || repositories.length === 0) {
            ...
            this.logger.info(`No cached repository for prompted webhook ${agentSessionId}, attempting fallback resolution`);
                ...
                if (repoId) {
                    ...
                    if (repo) {
                        ...
                        this.repositoryRouter
                            .getIssueRepositoryCache()
                            .set(issueId, [repoId]);
                        this.logger.info(`Recovered repository ${repoId} for issue ${issueId} from session manager`);
                    }
                }
            }
            ...
            if (!repositories || repositories.length === 0) {
                try {
                    ...
                    if (routingResult.type === "selected") {
                        repositories = routingResult.repositories;
                        this.repositoryRouter.getIssueRepositoryCache().set(issueId, routingResult.repositories.map((r) => r.id));
                        this.logger.info(`Recovered repositories [${repositories.map((r) => r.name).join(", ")}] for issue ${issueId} via fallback routing (${routingResult.routingMethod})`);
                    }
                }
                catch (error) {
                    this.logger.warn(`Fallback repository routing failed for prompted webhook ${agentSessionId}`, error);
                }
            }
            if (!repositories || repositories.length === 0) {
                // All recovery attempts failed - post visible feedback
                await this.agentSessionManager.createResponseActivity(agentSessionId, "I couldn't process your message because the session configuration was lost. Please create a new session by mentioning me (@cyrus) in a new comment with your prompt.");
                this.logger.warn(`Failed to recover repository for prompted webhook ${agentSessionId} - all fallback methods exhausted`);
                return;
            }
        }
     ...
     * @param linearWorkspaceId Linear workspace ID (from webhook.organizationId)
     */
    async handleIssueUnassigned(issue, linearWorkspaceId) {
        ...
        if (activeThreadCount > 0) {
            await this.postComment(issue.id, "I've been unassigned and am stopping work now.", linearWorkspaceId);
        }
        // Emit events
     ...
     */
    async handleClaudeMessage(sessionId, message, _repositoryId) {
     ...
     */
    attachCrashRecoveryHandler(runner, repository, linearAgentActivitySessionId, agentSessionManager) {
        // Guard: IAgentRunner doesn't require EventEmitter, so non-Claude/Gemini
        // runners (and test mocks) may not have .on(); skip silently.
    ...
    resolveSkillRepoPaths(repository, session) {
        ...
        if (repoPaths) {
            const paths = Object.values(repoPaths).filter((p) => typeof p === "string" && p.length > 0);
            if (paths.length > 0) {
    ...
    createRunnerForType(runnerType, config) {
        if (this.getMaintenanceMode().enabled) {
            throw new Error("Cyrus is in maintenance mode; refusing to start an agent runner");
        }
        return capRunnerStarts(this.buildRunnerForType(runnerType, config), this.runnerSlots);
    }
    buildRunnerForType(runnerType, config) {
        switch (runnerType) {
            case "claude": {
     ...
     */
    async moveIssueToStartedState(issue, linearWorkspaceId) {
        try {
            const issueTracker = this.issueTrackers.get(linearWorkspaceId);
            if (!issueTracker) {
                this.logger.warn(`No issue tracker found for workspace ${linearWorkspaceId}, skipping state update`);
                return;
            }
     ...
     */
    async postComment(issueId, body, linearWorkspaceId, parentId) {
        return this.activityPoster.postComment(issueId, body, linearWorkspaceId, parentId);
    }
    /**
     ...
     */
    async downloadIssueAttachments(issue, linearWorkspaceId, workspacePath) {
        const issueTracker = this.issueTrackers.get(linearWorkspaceId);
        return this.attachmentService.downloadIssueAttachments(issue, linearWorkspaceId, workspacePath, issueTracker);
    }
    /**
    ...
    async registerCyrusToolsMcpEndpoint() {
        ...
        if (typeof fastify.register !== "function" ||
            ...
            console.warn("[EdgeWorker] Skipping cyrus-tools MCP endpoint registration: Fastify instance does not support register/addHook");
            return;
        }
        fastify.addHook("onRequest", (request, _reply, done) => {
            ...
            if (!this.mcpConfigService.isAuthorizationValid(request.headers?.authorization)) {
        ...
        this.cyrusToolsMcpSessions.on("connected", (sessionId) => {
            console.log(`[EdgeWorker] cyrus-tools MCP session connected: ${sessionId}`);
        });
        this.cyrusToolsMcpSessions.on("terminated", (sessionId) => {
            console.log(`[EdgeWorker] cyrus-tools MCP session terminated: ${sessionId}`);
        });
        ...
        await fastify.register(streamableHttp, {
            ...
            createServer: async () => {
                const contextId = this.cyrusToolsMcpRequestContext.getStore()?.contextId;
                if (!contextId) {
                    throw new Error("Missing x-cyrus-mcp-context-id header for cyrus-tools MCP request");
                }
                const context = this.mcpConfigService.getContext(contextId);
                if (!context) {
                    throw new Error(`Unknown cyrus-tools MCP context '${contextId}'. Build MCP config before connecting.`);
                }
                const sdkServer = context.prebuiltServer ||
                    createCyrusToolsServer(context.linearClient, this.createCyrusToolsOptions(context.parentSessionId));
    ...
    getAllKnownSessions() {
        return [
            ...
            ...this.activeChatSessionHandlers.flatMap((handler) => handler.getAllChatSessions()),
        ];
    }
    ...
    handleChildSessionMapping(childSessionId, parentSessionId) {
        ...
        this.globalSessionRegistry.setParentSession(childSessionId, parentSessionId);
        console.log(`[EdgeWorker] Parent-child mapping registered in GlobalSessionRegistry`);
    }
    /**
     ...
     */
    async linkChildSessionToParentIssueSession(agentSessionId, issue, linearWorkspaceId) {
        if (!issue) {
            return;
        }
        ...
        try {
            ...
            const parentSessions = this.agentSessionManager.getSessionsByIssueId(parentIssueId);
            if (parentSessions.length === 0) {
                log.debug(`Parent issue ${parentIssueId} has no Cyrus session; no parent callback will be sent`);
                return;
            }
            const parentSession = parentSessions.reduce((latest, candidate) => candidate.updatedAt > latest.updatedAt ? candidate : latest);
            this.globalSessionRegistry.setParentSession(agentSessionId, parentSession.id);
            log.info(`Linked to parent session ${parentSession.id} via parent issue ${parentIssueId}; parent will be resumed when this session completes`);
        }
        catch (error) {
            log.warn(`Failed to link session to a parent issue session; continuing without parent callback`, error);
        }
    }
    async handleFeedbackDeliveryToChildSession(childSessionId, message) {
        ...
        const parentSessionId = this.globalSessionRegistry.getParentSessionId(childSessionId);
        ...
        this.handlePromptWithStreamingCheck(childSession, childRepo, childSessionId, this.agentSessionManager, feedbackPrompt, "", false, [], "give feedback to child", childWorkspaceId)
            .then(() => {
    ...
    async buildNewSessionPrompt(input) {
        ...
        if (!input.isMentionTriggered || input.isLabelBasedPromptRequested) {
            const result = await this.promptBuilder.determineSystemPromptFromLabels(input.labels || [], repositories);
        ...
        // cannot invoke.
        const skillsContext = this.buildSkillSessionContext(repositories[0], input.fullIssue, input.session);
...
${input.userComment}
    ...
    async buildIssueContextForPromptAssembly(issue, repositories, promptType, attachmentManifest, guidance, agentSession, resolvedBaseBranches, workspaceRepoPaths) {
        ...
        if (promptType === "label-based" ||
            ...
            return this.promptBuilder.buildLabelBasedPrompt(issue, repositories, attachmentManifest, guidance, resolvedBaseBranches);
        }
    ...
    buildAllowedTools(repositories, promptType) {
        return this.toolPermissionResolver.buildAllowedTools(repositories, promptType);
    }
    /**
     * Get Agent Sessions for an issue
     */
    getAgentSessionsForIssue(issueId, _repositoryId) {
     ...
     */
    async postMaintenanceNotice(webhook, message) {
        const issueTracker = this.issueTrackers.get(webhook.organizationId);
        if (!issueTracker) {
            this.logger.warn(`No issue tracker for workspace ${webhook.organizationId}; maintenance notice not posted`);
            return;
        }
        await this.postActivityDirect(issueTracker, {
    ...
    async loadPersistedState() {
        try {
            ...
            if (state) {
                ...
                this.logger.debug(`✅ Loaded persisted EdgeWorker state with ${Object.keys(state.agentSessions || {}).length} sessions`);
            }
        }
        catch (error) {
    ...
    async warmupRecentSessions(count = 30) {
        ...
        await Promise.all(candidates.map(async (session) => {
            try {
                ...
                const mcpConfigPath = resolveIssueMcpConfigPath(repo, this.config.linearMcpConfigs, this.mcpConfigService.buildMergedMcpConfigPath.bind(this.mcpConfigService));
    ...
    async savePersistedState() {
        try {
            ...
            this.logger.debug(`✅ Saved EdgeWorker state for ${Object.keys(state.agentSessions || {}).length} sessions`);
        }
        catch (error) {
     ...
     */
    async resumeInterruptedSessions() {
        console.log(`[EdgeWorker] Checking for interrupted sessions to resume...`);
        let resumedCount = 0;
        if (this.getMaintenanceMode().enabled) {
            console.log(`[EdgeWorker] Maintenance mode is on; not resuming interrupted sessions`);
            return;
        }
        const interruptedSessions = this.agentSessionManager.getInterruptedSessions();
        if (interruptedSessions.length === 0) {
            console.log(`[EdgeWorker] No interrupted sessions to resume`);
            return;
        }
        console.log(`[EdgeWorker] Found ${interruptedSessions.length} interrupted session(s)`);
        for (const session of interruptedSessions) {
            // Resolve the repository for this session
            const repoId = this.sessionRepositories.get(session.id);
            const repository = repoId ? this.repositories.get(repoId) : undefined;
            if (!repository) {
                console.warn(`[EdgeWorker] No repository found for interrupted session ${session.id}, skipping`);
                session.wasRunning = false;
                continue;
            }
            try {
                // Reset status to Active for recovery (it may be "complete" from last subroutine)
                this.agentSessionManager.resetSessionStatusForRecovery(session.id);
                // Build a recovery prompt with context from stored entries
                const summary = this.agentSessionManager.buildConversationSummary(session.id);
                const recoveryPrompt = summary
                    ? `${summary}\n\n---\n\nContinue where you left off.`
                    : "Continue the task you were working on.";
                console.log(`[EdgeWorker] Resuming interrupted session ${session.id} for issue ${session.issue?.identifier || session.issueId}`);
                // Post a thought activity to inform the user
                await this.agentSessionManager.createThoughtActivity(session.id, "Resuming after interruption. Let me continue where I left off.");
                // Resume the session with the recovery prompt
                await this.resumeAgentSession(session, repository, session.id, this.agentSessionManager, recoveryPrompt, "", // No attachment manifest for recovery
                false);
                resumedCount++;
            }
            catch (error) {
                console.error(`[EdgeWorker] Failed to resume interrupted session ${session.id}:`, error);
    ...
    restoreMappings(state) {
        ...
        if (state.agentSessions && state.agentSessionEntries) {
            this.agentSessionManager.restoreState(state.agentSessions, state.agentSessionEntries);
            ...
            if (state.issueRepositoryCache) {
                for (const [sessionId, session] of Object.entries(state.agentSessions)) {
            ...
            this.logger.debug(`Restored ${Object.keys(state.agentSessions).length} sessions`);
        }
        ...
        if (state.childToParentAgentSession) {
            ...
            this.logger.debug(`Restored ${entries.length} child-to-parent agent session mappings`);
        }
     ...
     */
    async postInstantAcknowledgment(sessionId, linearWorkspaceId) {
        return this.activityPoster.postInstantAcknowledgment(sessionId, linearWorkspaceId);
    }
    /**
     * Post parent resume acknowledgment thought when parent session is resumed from child
     */
    async postParentResumeAcknowledgment(sessionId, linearWorkspaceId) {
        return this.activityPoster.postParentResumeAcknowledgment(sessionId, linearWorkspaceId);
    }
    /**
    ...
    async handlePromptWithStreamingCheck(session, repository, sessionId, agentSessionManager, promptBody, attachmentManifest, isNewSession, additionalAllowedDirs, logContext, linearWorkspaceId, commentAuthor, commentTimestamp) {
        ...
        if (existingRunner?.isRunning() &&
            ...
            try {
                ...
                log.warn(`Streaming message rejected for ${sessionId}; falling back to resume (${logContext})`, { error: error instanceof Error ? error.message : String(error) });
            }
        }
        // Not streaming (or streaming was rejected) - resume/start session
        log.debug(`Resuming Claude session for ${sessionId} (${logContext})`);
        await this.resumeAgentSession(session, repository, sessionId, agentSessionManager, promptBody, attachmentManifest, isNewSession, additionalAllowedDirs, linearWorkspaceId, undefined, // maxTurns
     ...
     */
    async postSystemPromptSelectionThought(sessionId, labels, linearWorkspaceId, repositoryId) {
        return this.activityPoster.postSystemPromptSelectionThought(sessionId, labels, linearWorkspaceId, repositoryId);
    }
    /**
     ...
     * @param isNewSession Whether this is a new session
     */
    async resumeAgentSession(session, repository, sessionId, agentSessionManager, promptBody, attachmentManifest = "", isNewSession = false, additionalAllowedDirectories = [], linearWorkspaceId, maxTurns, commentAuthor, commentTimestamp) {
        const log = this.logger.withContext({ sessionId });
        ...
        if (existingRunner?.isRunning() &&
            ...
            try {
                ...
                log.warn(`Streaming message rejected for ${sessionId}; falling back to resume`, { error: error instanceof Error ? error.message : String(error) });
            }
        }
        ...
        const resolvedWorkspaceId = linearWorkspaceId ?? requireLinearWorkspaceId(repository);
        const fullIssue = await this.fetchFullIssueDetails(issueIdForResume, resolvedWorkspaceId);
        if (!fullIssue) {
        ...
        const hasOpenCodeSession = !isNewSession && Boolean(session.opencodeSessionId);
        ...
        const allowedDirectories = [
            ...new Set([
                ...
                ...this.gitService.getGitMetadataDirectoriesForWorkspace(session.workspace),
            ]),
        ];
        ...
        resolvedWorkspaceId, this.buildSkillSessionContext(repository, fullIssue, session));
        // Create the appropriate runner based on session state
        const runner = this.createRunnerForType(runnerType, runnerConfig);
        // Handle runner crash recovery (auto-resume on unexpected errors)
        this.attachCrashRecoveryHandler(runner, repository, sessionId, agentSessionManager);
        // Handle resume-failed event (stale session recovery)
        // This occurs when the persisted session ID no longer exists (e.g., after process restart)
        if (resumeSessionId && runnerType === "claude") {
            runner.on("resume-failed", async (staleSessionId) => {
                console.log(`[resumeAgentSession] Resume failed for session ${sessionId}, stale ID: ${staleSessionId}`);
                ...
                const contextSummary = agentSessionManager.buildConversationSummary(sessionId);
                // Prepare recovery prompt with context
                const recoveryPrompt = contextSummary
                    ? `${contextSummary}\n\n---\n\nContinuing with: ${promptBody}`
                    : promptBody;
                // Post thought to inform user about recovery
                await agentSessionManager.createThoughtActivity(sessionId, "Session interrupted - reconstructing context and continuing...");
                // Retry with a fresh session (recursively call resumeAgentSession with isNewSession=true)
                try {
                    await this.resumeAgentSession(session, repository, sessionId, agentSessionManager, recoveryPrompt, attachmentManifest, true, // Force new session (no resumeSessionId)
                    additionalAllowedDirectories, resolvedWorkspaceId, maxTurns, commentAuthor, commentTimestamp);
                    console.log(`[resumeAgentSession] Successfully recovered session ${sessionId} with fresh context`);
                }
                catch (retryError) {
                    console.error(`[resumeAgentSession] Failed to recover session ${sessionId}:`, retryError);
                    await agentSessionManager.createErrorActivity(sessionId, "Failed to recover session after interruption. Please try again.");
                }
            });
        }
        // Store runner
     ...
     */
    async postInstantPromptedAcknowledgment(sessionId, linearWorkspaceId, isStreaming) {
        return this.activityPoster.postInstantPromptedAcknowledgment(sessionId, linearWorkspaceId, isStreaming);
    }
    /**
     * Get the platform type for a workspace's issue tracker.
     */
    getRepositoryPlatform(linearWorkspaceId) {
        try {
     ...
     */
    async fetchFullIssueDetails(issueId, linearWorkspaceId) {
        const issueTracker = this.issueTrackers.get(linearWorkspaceId);
        if (!issueTracker) {
            this.logger.warn(`No issue tracker found for workspace ${linearWorkspaceId}`);
            return null;
        }
        try {
     ...
     */
    buildOAuthConfig(linearWorkspaceId) {
        ...
        if (!workspaceConfig?.linearRefreshToken) {
            this.logger.warn(`No refresh token for workspace ${linearWorkspaceId}, token refresh disabled`);
            return undefined;
        }
        // Get workspace name from workspace-level config
        const workspaceName = this.config.linearWorkspaces?.[linearWorkspaceId]?.linearWorkspaceName ||
            linearWorkspaceId;
        return {
            clientId,
            clientSecret,
            ...
            config.linearWorkspaces[tokens.linearWorkspaceId] = {
                ...
                ...(tokens.linearRefreshToken
                    ...
                    : config.linearWorkspaces[tokens.linearWorkspaceId]
                        ?.linearRefreshToken
                        ? {
                            linearRefreshToken: config.linearWorkspaces[tokens.linearWorkspaceId]
                                .linearRefreshToken,
                        }
                        : {}),
                ...(tokens.linearWorkspaceName
                    ...
                    : config.linearWorkspaces[tokens.linearWorkspaceId]
                        ?.linearWorkspaceName
                        ? {
                            linearWorkspaceName: config.linearWorkspaces[tokens.linearWorkspaceId]
                                .linearWorkspaceName,
                        }
                        : {}),
            };
