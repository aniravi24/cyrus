export class ActivityPoster {
    ...
    async postThoughtActivity(sessionId, workspaceId, body) {
    ...
    async postRepoSetupHookActivity(sessionId, workspaceId, event) {
        const issueTracker = this.issueTrackers.get(workspaceId);
        if (!issueTracker) {
            this.logger.warn(`No issue tracker found for workspace ${workspaceId}`);
            return;
    ...
    formatRepoSetupHookResult(event) {
    ...
    formatRepoSetupHookFailureHint(event) {
