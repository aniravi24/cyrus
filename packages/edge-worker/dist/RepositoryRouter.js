...
export class RepositoryRouter {
     ...
     */
    getCachedRepositories(issueId, repositoriesMap) {
        ...
        if (invalidIds.length > 0) {
            this.logger.warn(`Cached repositories [${invalidIds.join(", ")}] no longer exist, cleaning cache`);
            if (resolvedRepos.length === 0) {
                this.issueRepositoryCache.delete(issueId);
                return null;
            }
            // Update cache to only contain valid IDs
            this.issueRepositoryCache.set(issueId, resolvedRepos.map((r) => r.id));
        }
        this.logger.debug(`Using cached repositories [${resolvedRepos.map((r) => r.name).join(", ")}] for issue ${issueId}`);
    ...
    async determineRepositoryForWebhook(webhook, repos) {
        ...
        if (issueId) {
            ...
            if (activeRepos.length > 0) {
                this.logger.info(`Repositories selected: [${activeRepos.map((r) => r.name).join(", ")}] (existing active session)`);
                return {
                    type: "selected",
        ...
        const descriptionTagResult = await this.findRepositoriesByDescriptionTag(issueId, workspaceRepos, workspaceId);
        if (descriptionTagResult.repositories.length > 0) {
            this.logger.info(`Repositories selected: [${descriptionTagResult.repositories.map((r) => r.name).join(", ")}] (description-tag routing)`);
            if (descriptionTagResult.baseBranchOverrides.size > 0) {
                const overrideEntries = Array.from(descriptionTagResult.baseBranchOverrides.entries())
                    .map(([id, branch]) => `${id}→${branch}`)
                    .join(", ");
                this.logger.info(`Base branch overrides from description tags: ${overrideEntries}`);
            }
            return {
                type: "selected",
                repositories: descriptionTagResult.repositories,
                baseBranchOverrides: descriptionTagResult.baseBranchOverrides.size > 0
        ...
        const labelMatchedRepos = await this.findRepositoriesByLabels(issueId, workspaceRepos, workspaceId);
        if (labelMatchedRepos.length > 0) {
            this.logger.info(`Repositories selected: [${labelMatchedRepos.map((r) => r.name).join(", ")}] (label-based routing)`);
            return {
                type: "selected",
        ...
        this.logger.info(`No routing match for ${workspaceRepos.length} workspace repositories - requesting user selection`);
        return { type: "needs_selection", workspaceRepos };
    }
    /**
     * Find all repositories matching routing labels
     */
    async findRepositoriesByLabels(issueId, repos, workspaceId) {
        if (!issueId)
            return [];
     ...
     */
    async findRepositoriesByDescriptionTag(issueId, repos, workspaceId) {
        if (!issueId)
        ...
        try {
            ...
            if (repoTags.length === 0)
                return { repositories: [], baseBranchOverrides: new Map() };
            this.logger.info(`Found repo tags in issue description: [${repoTags.map((t) => (t.branch ? `${t.repo}#${t.branch}` : t.repo)).join(", ")}]`);
            const matched = [];
            const matchedIds = new Set();
            const baseBranchOverrides = new Map();
            for (const repoTag of repoTags) {
                for (const repo of repos) {
                    if (matchedIds.has(repo.id))
                        continue;
                    let isMatch = false;
                    ...
                    if (repo.githubUrl?.endsWith(`/${repoTag.repo}`) ||
                        repo.githubUrl?.endsWith(`/${repoTag.repo}.git`) ||
                        repo.gitlabUrl?.endsWith(`/${repoTag.repo}`) ||
                        repo.gitlabUrl?.endsWith(`/${repoTag.repo}.git`)) {
                        this.logger.debug(`Matched repo tag "${repoTag.repo}" to repository ${repo.name} via hosting URL`);
                        isMatch = true;
                    }
                    // Match by repository name (exact match, case-insensitive)
                    if (!isMatch &&
                        repo.name.toLowerCase() === repoTag.repo.toLowerCase()) {
                        this.logger.debug(`Matched repo tag "${repoTag.repo}" to repository ${repo.name} via name`);
                        isMatch = true;
                    }
                    // Match by repository ID
                    if (!isMatch && repo.id === repoTag.repo) {
                        this.logger.debug(`Matched repo tag "${repoTag.repo}" to repository ${repo.name} via ID`);
                        isMatch = true;
                    }
                    if (isMatch) {
                        matched.push(repo);
                        ...
                        if (repoTag.branch) {
                            ...
                            this.logger.debug(`Base branch override for ${repo.name}: ${repoTag.branch}`);
                        }
                    }
                }
            }
            if (matched.length === 0) {
                this.logger.debug(`No repositories matched [repo=...] tags: [${repoTags.map((t) => t.repo).join(", ")}]`);
            }
     ...
     */
    parseRepoTagsFromDescription(description) {
        const tags = [];
