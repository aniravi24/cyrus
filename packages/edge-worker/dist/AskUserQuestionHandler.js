...
export class AskUserQuestionHandler {
    ...
    async handleAskUserQuestion(input, linearAgentSessionId, organizationId, signal) {
        ...
        if (!input.questions || input.questions.length !== 1) {
            this.logger.error(`Invalid input: expected exactly 1 question, got ${input.questions?.length ?? 0}`);
            return {
                answered: false,
        ...
        this.logger.debug(`Handling question for session ${linearAgentSessionId}: ${question.header}`);
        ...
        if (!issueTracker) {
            this.logger.error(`No issue tracker found for organization ${organizationId}`);
            return {
                answered: false,
        ...
        if (this.pendingQuestions.has(linearAgentSessionId)) {
            this.logger.warn(`Replacing existing pending question for session ${linearAgentSessionId}`);
        ...
        try {
            ...
            this.logger.debug(`Posted elicitation with ${options.length} options for session ${linearAgentSessionId}`);
        }
        catch (error) {
        ...
        return new Promise((resolve) => {
            ...
            const abortHandler = () => {
                this.logger.debug(`Question cancelled for session ${linearAgentSessionId}`);
    ...
    handleUserResponse(linearAgentSessionId, selectedValue) {
        ...
        if (!pendingQuestion) {
            this.logger.debug(`No pending question found for session ${linearAgentSessionId}`);
            return false;
        }
        this.logger.debug(`User responded to question for session ${linearAgentSessionId}: ${selectedValue}`);
    ...
    cancelPendingQuestion(linearAgentSessionId, reason) {
        const pendingQuestion = this.pendingQuestions.get(linearAgentSessionId);
        if (pendingQuestion) {
            this.logger.debug(`Cancelling pending question for session ${linearAgentSessionId}: ${reason}`);
            pendingQuestion.resolve({
                answered: false,
                message: reason,
            });
