...
export class SessionSemaphore {
    ...
    activeCount = 0;
    waiters = [];
    constructor(limit, onQueued) {
        this.limit = limit;
        this.onQueued = onQueued;
        if (Number.isNaN(limit) || limit < 1) {
            throw new Error(`SessionSemaphore limit must be >= 1 or Infinity, got ${limit}`);
        }
    }
    get active() {
    ...
    acquire() {
        ...
        return new Promise((resolve) => {
            ...
            this.onQueued?.(`Session start queued: ${this.activeCount} running at the ` +
 ...
 */
export function capRunnerStarts(runner, semaphore) {
    ...
    return new Proxy(runner, {
        get(target, property, receiver) {
            if (property === "start") {
                return (prompt) => gate(() => target.start(prompt));
            }
            if (property === "startStreaming" &&
...
//# sourceMappingURL=RunnerConcurrency.js.map