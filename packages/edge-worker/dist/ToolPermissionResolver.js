...
export class ToolPermissionResolver {
    ...
    buildDisallowedTools(repositories, promptType) {
        ...
        if (perRepoTools.length === 1) {
            ...
            intersection = [...firstSet].filter((tool) => perRepoTools.every((repoTools) => repoTools.includes(tool)));
        }
        if (intersection.length > 0) {
            const repoNames = repoArray.map((r) => r.name).join(", ");
            this.logger.debug(`Disallowed tools for [${repoNames}]: ${intersection.length} tools (intersection of ${repoArray.length} repo(s))`);
        }
        return intersection;
    }
    /**
     ...
     */
    buildDisallowedToolsForRepo(repository, promptType) {
        const effectivePromptType = promptType === "graphite-orchestrator" ? "orchestrator" : promptType;
