...
export class DefaultSkillsDeployer {
    ...
    async ensureDeployed() {
        if (await this.exists(this.deployedPluginPath)) {
            this.logger.debug(`Default skills plugin already exists at ${this.deployedPluginPath}`);
            return;
        }
        if (!(await this.exists(this.bundledSkillsPath))) {
            this.logger.warn(`Bundled skills not found at ${this.bundledSkillsPath} — cannot deploy defaults`);
            return;
        }
        ...
        this.logger.info(`Deployed default skills to ${this.deployedPluginPath} (${deployedCount} skills)`);
    }
    async exists(path) {
        try {
            await access(path);
...
//# sourceMappingURL=DefaultSkillsDeployer.js.map