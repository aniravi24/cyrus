...
export class SlackEventTransport extends EventEmitter {
    ...
    static DEDUP_TTL_MS = 10 * 60 * 1000;
    ...
    resolveVerification() {
        ...
        if (isExternalHost && hasSlackSigningSecret) {
            this.logger.info("Runtime switch: SLACK_SIGNING_SECRET detected, using direct Slack signature verification");
     ...
     */
    register() {
        this.config.fastifyServer.post("/slack-webhook", {
            config: {
                rawBody: true,
            },
        }, async (request, reply) => {
        ...
        this.logger.info(`Registered POST /slack-webhook endpoint (${this.config.verificationMode} mode)`);
    }
    /**
     * Handle webhook using Slack signing secret (direct from Slack)
     */
    async handleDirectWebhook(request, reply, secret) {
        ...
        try {
            ...
            const isValid = this.verifySlackSignature(body, timestamp, signature, secret);
            if (!isValid) {
     ...
     */
    processAndEmitEvent(request, reply, upstreamGated) {
        ...
        if (eventType !== "app_mention" && eventType !== "message") {
            this.logger.debug(`Ignoring unsupported event type: ${eventType ?? "unknown"}`);
            reply.code(200).send({ success: true, ignored: true });
            return;
        }
        ...
        if (event.type === "message" &&
            ...
            !this.config.isThreadFollowingEnabled()) {
            this.logger.debug(`Slack thread-following disabled; ignoring message event (channel ${event.channel})`);
            reply.code(200).send({ success: true, ignored: true });
            return;
        }
        ...
        if (this.isDuplicateMessage(dedupKey)) {
            this.logger.debug(`Ignoring duplicate Slack event for ${dedupKey} (already processed)`);
            reply.code(200).send({ success: true, ignored: true });
            return;
        }
        ...
        this.logger.info(`Received ${event.type} webhook (event: ${envelope.event_id}, channel: ${event.channel})`);
     ...
     */
    shouldEmitMessageEvent(event) {
        if (event.bot_id) {
            this.logger.debug(`Ignoring Slack message from bot ${event.bot_id} (channel ${event.channel})`);
            return false;
        }
        if (event.subtype) {
            this.logger.debug(`Ignoring Slack message with subtype "${event.subtype}" (channel ${event.channel})`);
            return false;
        }
        if (!event.thread_ts) {
            this.logger.debug(`Ignoring non-threaded Slack message (channel ${event.channel})`);
            return false;
        }
        return true;
    }
...
//# sourceMappingURL=SlackEventTransport.js.map