...
export class SlackReactionService {
    ...
    async callReactionApi(method, params, ignorableError) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`[SlackReactionService] ${method} failed: ${response.status} ${response.statusText} - ${errorBody}`);
        }
        ...
        if (!responseBody.ok && responseBody.error !== ignorableError) {
            throw new Error(`[SlackReactionService] Slack API error: ${responseBody.error ?? "unknown"}`);
        }
    }
}
//# sourceMappingURL=SlackReactionService.js.map