...
export class CLIIssueTrackerService extends EventEmitter {
     ...
     */
    async terminateIssue(issueId, action) {
        const issueData = this.state.issues.get(issueId);
        if (!issueData) {
            throw new Error(`Issue ${issueId} not found`);
        }
        ...
        if (action === "completed" || action === "canceled") {
            const targetStateId = action === "completed" ? "state-done" : "state-canceled";
            const targetState = this.state.workflowStates.get(targetStateId);
            if (!targetState) {
                throw new Error(`Seeded workflow state ${targetStateId} not found — call seedDefaultData() first`);
            }
