...
export class CLIRPCServer {
    ...
    async handleCommand(method, params, requestId) {
        switch (method) {
            ...
            case "terminateIssue":
                return this.handleTerminateIssue(params, requestId);
            default:
                return {
                    jsonrpc: "2.0",
                    error: {
     ...
     */
    async handleTerminateIssue(params, requestId) {
        const { issueId, action } = params ?? {};
        if (!issueId) {
            return {
                jsonrpc: "2.0",
                error: {
        ...
        if (action !== "completed" &&
            ...
            return {
                ...
                error: {
                    code: RPCErrorCodes.INVALID_PARAMS,
                    message: "Invalid action: must be one of 'completed', 'canceled', 'deleted'",
                },
                id: requestId,
            };
        }
        try {
            const identifier = await this.config.issueTracker.terminateIssue(issueId, action);
            return {
                jsonrpc: "2.0",
                result: {
                    success: true,
