...
export class ClaudeMessageFormatter {
    ...
    formatTaskParameter(toolName, toolInput) {
        try {
            ...
            console.error("[ClaudeMessageFormatter] Failed to format Task parameter:", error);
            return JSON.stringify(toolInput);
        }
    }
    /**
        ...
        try {
            switch (toolName) {
                ...
                case "↪ TaskList":
                    ...
                    return this.formatTaskParameter(toolName.replace("↪ ", ""), toolInput);
                case "ToolSearch":
                case "↪ ToolSearch": {
                    ...
                    if (query.startsWith("select:")) {
                        ...
                        const label = toolNames.length === 1 ? "tool schema" : "tool schemas";
        ...
        try {
            switch (toolName) {
                ...
                case "↪ ToolSearch": {
                    ...
                    const looksLikeToolNames = lines.length > 0 && lines.every((line) => /^[\w.-]+$/.test(line));
