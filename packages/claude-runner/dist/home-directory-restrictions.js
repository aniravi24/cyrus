...
export function buildHomeDirectoryDisallowedTools(cwd, additionalAllowedPaths = []) {
    ...
    const allRelPaths = [cwd, ...additionalAllowedPaths]
        .map((p) => resolve(p))
    ...
    function processDir(dir, relevantPaths) {
        // Build the set of child names that lead toward at least one allowed path.
        const allowedNames = new Set(relevantPaths.map((segs) => segs[0]).filter(Boolean));
        let entries;
        try {
...
//# sourceMappingURL=home-directory-restrictions.js.map