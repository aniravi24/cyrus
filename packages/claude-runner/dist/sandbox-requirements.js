 ...
 */
export function checkLinuxSandboxRequirements() {
 ...
 */
export function logSandboxRequirementFailures(result, logger) {
    if (result.supported || hasLoggedFailures) {
        return;
    }
    hasLoggedFailures = true;
    logger.warn("Linux sandbox requirements are not met — skipping CLAUDE_CODE_SUBPROCESS_ENV_SCRUB.");
    logger.warn("Claude sessions will continue, but subprocess env scrubbing will be disabled until these are resolved:");
 ...
 */
export function resetSandboxRequirementsCacheForTesting() {
...
function runBwrapSandboxProbe() {
    ...
    const firstStderrLine = stderr && stderr.length > 0 ? stderr.split("\n")[0] : undefined;
    const reason = firstStderrLine ?? `bwrap exited with status ${probe.status ?? "unknown"}`;
    return { ok: false, reason };
}
function buildBwrapSandboxResolution() {
    return [
...
//# sourceMappingURL=sandbox-requirements.js.map