...
export class ClaudeRunner extends EventEmitter {
    ...
    createCanUseToolCallback() {
        return async (toolName, input, options) => {
            ...
            console.log(`[ClaudeRunner] Intercepted AskUserQuestion tool call (toolUseID: ${options.toolUseID})`);
            ...
            if (askInput.questions.length !== 1) {
                console.log(`[ClaudeRunner] Rejecting AskUserQuestion with ${askInput.questions.length} questions (only 1 allowed)`);
                return {
                    behavior: "deny",
            ...
            if (!this.config.onAskUserQuestion) {
                console.error("[ClaudeRunner] onAskUserQuestion callback not configured");
                return {
                    behavior: "deny",
            ...
            if (!sessionId) {
                console.error("[ClaudeRunner] Cannot handle AskUserQuestion without session ID");
                return {
                    behavior: "deny",
            ...
            try {
                ...
                console.log(`[ClaudeRunner] Delegating AskUserQuestion to callback for session ${sessionId}`);
                const result = await this.config.onAskUserQuestion(askInput, sessionId, options.signal);
                if (result.answered && result.answers) {
                    console.log(`[ClaudeRunner] User answered AskUserQuestion for session ${sessionId}`);
                    ...
                    console.log(`[ClaudeRunner] User denied AskUserQuestion for session ${sessionId}: ${result.message}`);
                    return {
                        behavior: "deny",
                ...
                console.error(`[ClaudeRunner] Error handling AskUserQuestion: ${errorMessage}`);
                return {
                    behavior: "deny",
    ...
    async startWithPrompt(stringPrompt, streamingInitialPrompt) {
        ...
        try {
            ...
            if (processedDisallowedTools) {
                console.log(`[ClaudeRunner] Disallowed tools configured:`, processedDisallowedTools);
            }
            ...
            if (this.config.workingDirectory) {
                ...
                if (existsSync(autoMcpPath)) {
                    try {
                        ...
                        console.log(`[ClaudeRunner] Auto-detected MCP config at ${autoMcpPath}`);
                    }
                    catch (_error) {
                        // Silently skip invalid .mcp.json files (could be test fixtures, etc.)
                        console.log(`[ClaudeRunner] Skipping invalid .mcp.json at ${autoMcpPath}`);
                    }
                }
            }
            ...
            if (this.config.allowedDirectories) {
                console.log(`[ClaudeRunner] Allowed directories configured:`, this.config.allowedDirectories);
            }
            const queryOptions = {
            ...
            if (isAbortError) {
                ...
                console.log(`[ClaudeRunner] Resume failed - session no longer exists: ${this.config.resumeSessionId}`);
    ...
    processMessage(message) {
        switch (message.type) {
            ...
            case "system":
                ...
                break;
            default:
                console.log(`[ClaudeRunner] Unhandled message type: ${message.type}`);
        }
    }
    /**
     * Load environment variables from repository .env file
    ...
    loadRepositoryEnv(workingDirectory) {
        try {
            ...
            if (existsSync(envPath)) {
                ...
                if (result.error) {
                    console.warn(`[ClaudeRunner] Failed to parse .env file:`, result.error);
                }
