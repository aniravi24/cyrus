...
export function createCyrusToolsServer(linearApiToken, options = {}) {
    ...
    const giveFeedbackTool = tool("linear_agent_give_feedback", "Provide feedback to a child agent session to continue its processing.", {
        ...
        if (options.onFeedbackDelivery) {
            ...
            try {
                const delivered = await options.onFeedbackDelivery(agentSessionId, message);
                if (delivered) {
    ...
    const setIssueRelationTool = tool("linear_set_issue_relation", "Create a relationship between two Linear issues. Use this to set 'blocks', 'related', or 'duplicate' relationships. For Graphite stacking workflows, use 'blocks' type where the blocking issue is the one that must be completed first.", {
        issueId: z
            .string()
        ...
        try {
            console.log(`Creating ${type} relation: ${issueId} -> ${relatedIssueId} (${issueId} blocks ${relatedIssueId})`);
            ...
            console.log(`Created ${type} relation: ${issue.identifier} ${type} ${relatedIssue.identifier}`);
            return {
                content: [
                    {
                        type: "text",
    ...
    const getAgentSessionsTool = tool("linear_get_agent_sessions", "Get all agent sessions. Returns a paginated list of agent sessions with their details including status, timestamps, and associated issues.", {
        first: z
            .number()
            .optional()
    ...
    const getAgentSessionTool = tool("linear_get_agent_session", "Get a single agent session by ID. Returns detailed information about the agent session including its status, timestamps, associated issue, and metadata.", {
        sessionId: z
            .string()
