 ...
 */
export function createSoraToolsServer(options) {
    const { apiKey, outputDirectory = process.cwd() } = options;
    // Initialize OpenAI client with default endpoint
    const client = new OpenAI({
        apiKey,
    });
    const generateVideoTool = tool("sora_generate_video", "Generate a video using Sora 2. Supports text-to-video and image-to-video generation. For image-to-video, the reference image must match the target video resolution (width x height). Returns a job ID to poll for completion.", {
        prompt: z
            .string()
    ...
    const checkStatusTool = tool("sora_check_status", "Check the status of a Sora video generation job. Poll this endpoint until status is 'completed' or 'failed'.", {
        jobId: z
            .string()
    ...
    const getVideoTool = tool("sora_get_video", "Download a completed Sora video and save it to disk. Returns the local file path.", {
        jobId: z
            .string()
